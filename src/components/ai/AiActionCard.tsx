    ...
    <div className="not-prose rounded-xl border border-primary/30 bg-primary/5 p-3 space-y-2 text-sm">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Field label="日付">
            <Input
              type="date"
              value={draft.date}
              onChange={(e) => set({ date: e.target.value } as any)}
              className="h-8"
            />
          </Field>
          <Field label="教科（必須）">
            <Select
              ...
              onValueChange={(value) => set({ subject: value } as any)}
            >
          ...
          <Field label="時間（分）">
            <Input
              ...
              max={400}
              value={draft.minutes}
              onChange={(e) => set({ minutes: Number(e.target.value) } as any)}
              className="h-8"
            />
          </Field>
          <Field label="教材（任意）">
            <Select
              value={draft.material ?? "none"}
              onValueChange={(value) => set({ material: value === "none" ? null : value } as any)}
            >
              <SelectTrigger className="h-8">
          ...
          <div className="col-span-2">
            <Field label="内容">
              <Input
                value={draft.content}
                onChange={(e) => set({ content: e.target.value } as any)}
                className="h-8"
              />
            </Field>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <Field label="目標時間（分）">
            <Input
              ...
              onChange={(e) => set({ target_minutes: Number(e.target.value) } as any)}
              className="h-8"
            />
          </Field>
          <Field label="期限（任意）">
            <Input
              ...
              onChange={(e) => set({ deadline: e.target.value || null } as any)}
              className="h-8"
            />
          </Field>
        </div>
      ...
      <div className="flex gap-2 pt-1">
        <Button
          size="sm"
          onClick={approve}
          disabled={busy || (draft.kind === "add_study_log" && !draft.subject)}
        >
          {busy ? (
