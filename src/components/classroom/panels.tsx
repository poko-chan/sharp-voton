          ...
          <Card key={m.user_id} className="p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-xs text-muted-foreground">
                合計 {Math.floor(total / 60)}h {total % 60}m / {items.length}件
              </div>
            </div>
            <div className="mt-2 space-y-1 max-h-60 overflow-y-auto">
              {items.slice(0, 30).map((l, i) => (
                <div
                  key={i}
                  className="text-xs flex items-center gap-2 p-1.5 rounded hover:bg-muted/30"
                >
                  <span className="text-muted-foreground w-20">{l.date}</span>
                  {l.subjects && (
                    <span
                      className="px-1.5 py-0.5 rounded text-[10px]"
                      style={{ background: l.subjects.color + "33", color: l.subjects.color }}
                    >
                      {l.subjects.name}
                    </span>
                  )}
...
export function FilesTab({
  ...
  return (
    <ClassFilesPanel
      classId={classId}
      isTeacher={isTeacher}
      canUpload={isTeacher || perm.can_upload_files}
    />
  );
}
...
export function ClassChatTab({ classId, userId }: { classId: string; userId?: string }) {
  ...
  const send = async () => {
    ...
    const { error } = await supabase
      .from("class_chat_messages")
      .insert({ class_id: classId, sender_id: userId, body: text.trim() });
          ...
          .map((m) => {
            ...
            return (
              <div key={m.id} className={`flex gap-2 ${mine ? "flex-row-reverse" : ""}`}>
                <div
                  className={`max-w-[70%] rounded-lg px-3 py-2 text-sm ${mine ? "bg-primary text-primary-foreground" : "bg-muted"}`}
                >
                  {!mine && (
