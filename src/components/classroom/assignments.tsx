...
export function CreateAssignment({
  ...
  const addQ = () =>
    setQuiz((q) => [
      ...q,
      { id: crypto.randomUUID(), type: "short", question: "", answer: "", points: 1, options: [] },
    ]);
  ...
  const submit = async () => {
    ...
    const payload: any = {
      ...
      quiz_answer_key:
        kind === "quiz" ? quiz.map((q: any) => ({ id: q.id, answer: q.answer })) : null,
    };
        ...
        <div className="space-y-3">
          ...
          {kind === "quiz" && (
            <div className="space-y-2 border rounded p-3">
              ...
              {quiz.length === 0 && (
                <p className="text-xs text-muted-foreground">問題を追加してください</p>
              )}
              {quiz.map((q, idx) => (
                <QuizQuestionEditor
                  key={q.id}
  ...
  return (
    <div className="border rounded p-2 space-y-2 bg-muted/20">
      ...
      <Select
        ...
        onValueChange={(v) =>
          onChange({
            ...q,
            type: v,
            options: v === "choice" ? (q.options?.length ? q.options : ["", ""]) : [],
          })
        }
      >
        ...
        <div className="space-y-1">
          {(q.options ?? []).map((op: string, i: number) => (
            <div key={i} className="flex items-center gap-1">
              ...
              <button
                onClick={() =>
                  onChange({ ...q, options: q.options.filter((_: any, j: number) => j !== i) })
                }
              >
                <X className="h-3 w-3" />
              </button>
      ...
      <Input
        ...
        placeholder="正解（短答=正解文字列 / 選択=選択肢の文字列と完全一致）"
        className="h-7 text-xs"
      />
      <Input
        type="number"
...
export function AssignmentDialog({ assignment, isTeacher, members, onClose }: any) {
  ...
  const load = async () => {
    ...
    if (isTeacher) {
      ...
      if (qa?.answers) {
        ...
        (qa.answers as any[]).forEach((a: any) => {
  ...
  const submit = async () => {
    ...
    const { error } = mySub
      ? await supabase
          .from("submissions")
          .update({ content, attachments: files, submitted_at: new Date().toISOString() })
          .eq("id", mySub.id)
  ...
  const submitQuizAns = async () => {
    const answers = (assignment.quiz_questions ?? []).map((q: any) => ({
      questionId: q.id,
      answer: quizAns[q.id] ?? "",
    }));
    try {
      ...
      toast.success(
        `自動採点: ${r.earnedPts}/${r.totalPts}点（${r.score}/${assignment.max_points}）`,
      );
      load();
    } catch (e: any) {
      toast.error(e.message);
      ...
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          <div className="text-xs text-muted-foreground">
            {assignment.due_at
              ? `期限: ${new Date(assignment.due_at).toLocaleString("ja-JP")}`
              : "期限なし"}{" "}
            ・ 配点 {assignment.max_points}
            {assignment.allowed_file_types?.length > 0 && (
              <> ・ 提出形式: {assignment.allowed_file_types.join(", ")}</>
            )}
          </div>
          ...
          {!isTeacher && assignment.kind === "quiz" && (
            <Card className="p-4 space-y-3">
              ...
              {mySub?.graded_at && (
                <div className="p-2 rounded bg-emerald-500/10 border border-emerald-500/30 text-sm">
                  ...
                  <span className="font-bold">
                    {mySub.score}/{assignment.max_points} 点
                  </span>{" "}
                  ・ XP {mySub.xp_awarded}
                </div>
              )}
                ...
                return (
                  <div key={q.id} className="space-y-1">
                    ...
                    {q.type === "choice" ? (
                        ...
                        {(q.options ?? []).map((op: string, j: number) => (
                          <label key={j} className="flex items-center gap-2 text-sm">
                            <input
                              ...
                              name={q.id}
                              value={op}
                              checked={quizAns[q.id] === op}
                              onChange={() => setQuizAns({ ...quizAns, [q.id]: op })}
                            />
                            {op}
                          </label>
                        ))}
                      ...
                      <Input
                        value={quizAns[q.id] ?? ""}
                        onChange={(e) => setQuizAns({ ...quizAns, [q.id]: e.target.value })}
                      />
                    )}
                    {sub && (
                      <div
                        className={`text-xs ${sub.correct ? "text-emerald-600" : "text-red-600"}`}
                      >
              ...
              <Button onClick={submitQuizAns}>
                {mySub ? "再提出して再採点" : "提出して自動採点"}
              </Button>
            </Card>
          )}
              ...
              <FileUploader
                ...
                accept={
                  assignment.allowed_file_types?.length
                    ? assignment.allowed_file_types.map((t: string) => "." + t).join(",")
                    : undefined
                }
              />
              {files.length > 0 && (
          ...
          {isTeacher && (
            <Card className="p-4 space-y-3">
              <div className="font-semibold text-sm">
                提出一覧 ({allSubs.length}/{members.length}人)
              </div>
              {allSubs.length === 0 && (
                <p className="text-xs text-muted-foreground">まだ提出はありません</p>
              )}
              {allSubs.map((s) => {
    ...
    <div className="p-3 rounded border space-y-2">
      <div className="flex items-center justify-between">
        <div className="font-medium text-sm">
          {memberName}
          {isQuiz && <span className="ml-2 text-xs text-primary">自動採点</span>}
        </div>
        <div className="text-xs text-muted-foreground">
      ...
      {sub.content && (
        <p className="text-sm whitespace-pre-wrap bg-muted/30 p-2 rounded">{sub.content}</p>
      )}
        ...
        <div className="text-xs space-y-0.5 bg-muted/30 p-2 rounded">
          {sub.quiz_answers.detail.map((d: any, i: number) => (
            <div key={i} className={d.correct ? "text-emerald-600" : "text-red-600"}>
              {d.correct ? "✓" : "✗"} 回答: {d.userAnswer || "(無回答)"}{" "}
              {!d.correct && `／正解: ${d.expected}`}
            </div>
          ))}
        </div>
      )}
      ...
      {sub.graded_at && (
        <div className="text-[10px] text-emerald-600">採点済 ・ 付与XP {sub.xp_awarded}</div>
      )}
    </div>
  );
}
