    ...
    <div className="space-y-4 mt-4">
      <Card className="p-6 space-y-3 max-w-2xl">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold">お知らせを送信</h3>
        </div>
        <div>
          <Label>タイトル</Label>
        ...
        <p className="text-xs text-muted-foreground">
          未来の日時を指定すると、その時刻以降にユーザーから見えるようになります。
        </p>
              ...
              <tr key={a.id} className="border-t align-top">
                <td className="p-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span
                      className={`text-[10px] px-1.5 py-0.5 rounded border ${tagMeta(a.tag).className}`}
                    >
                      {tagMeta(a.tag).label}
                    </span>
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={editForm.showOnLogin}
                onChange={(e) => setEditForm({ ...editForm, showOnLogin: e.target.checked })}
              />
              ログイン画面にも表示する
            </label>
          </div>
...
export function FaqTab() {
  ...
  const move = async (id: string, dir: -1 | 1) => {
    ...
    await supabase
      .from("faq_entries")
      .update({ order_index: items[j].order_index })
      .eq("id", items[i].id);
    await supabase
      .from("faq_entries")
      .update({ order_index: items[i].order_index })
      .eq("id", items[j].id);
    load();
  };
    ...
    <div className="space-y-4 mt-4">
      <Card className="p-6 max-w-2xl space-y-3 border-emerald-500/30">
        <div className="flex items-center gap-2 text-emerald-600">
          ...
          <h3 className="font-semibold">FAQ を追加</h3>
        </div>
        <p className="text-xs text-muted-foreground">
          ログイン画面の「ヘルプ」ボタンから誰でも閲覧できます。
        </p>
        <div>
          <Label>質問</Label>
          <Input
        ...
        <div>
          ...
          <Textarea
            ...
            rows={4}
            placeholder="まずブラウザを最新にして…"
          />
        </div>
        <Button onClick={add} className="bg-emerald-600 hover:bg-emerald-700">
      ...
      <Card className="p-0 overflow-hidden max-w-3xl">
        <table className="w-full text-sm">
          <thead className="bg-muted">
            <tr className="text-left">
              <th className="p-3">質問/回答</th>
              <th className="p-3">公開</th>
              <th className="p-3">並び</th>
              <th className="p-3"></th>
            </tr>
          </thead>
          <tbody>
              ...
              <tr key={f.id} className="border-t align-top">
                <td className="p-3 space-y-1">
                  <Input
                    ...
                    onBlur={(e) => save(f.id, { question: e.target.value })}
                  />
                  <Textarea
                    rows={3}
                    defaultValue={f.answer}
                    onBlur={(e) => save(f.id, { answer: e.target.value })}
                  />
                </td>
                <td className="p-3">
                  <Switch
                    checked={f.published}
                    onCheckedChange={(v) => save(f.id, { published: v })}
                  />
                </td>
                <td className="p-3 space-x-1 whitespace-nowrap">
                  <Button size="sm" variant="outline" onClick={() => move(f.id, -1)}>
                    ↑
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => move(f.id, 1)}>
                    ↓
...
export function LoginBoardsTab() {
  ...
  const toggleOverlay = async (v: boolean) => {
    ...
    toast.success(
      v ? "ログイン演出をONにしました" : "ログイン演出をOFFにしました（以後/updatesで閲覧）",
    );
  };
            ...
            <div className="flex items-start gap-2">
              <div className="flex-1">
                <div className="font-semibold">
                  {b.title}{" "}
                  <span className="text-[10px] px-1.5 rounded bg-muted ml-1">
