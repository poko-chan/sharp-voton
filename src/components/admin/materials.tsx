...
export function MaterialsReviewTab() {
  ...
  const load = async () => {
    ...
    setPendingMaterials(
      (pm ?? []).map((m: any) => ({ ...m, profile: profiles[m.created_by] ?? null })),
    );
  ...
  const reviewMaterial = async (id: string, approve: boolean) => {
    const { error } = await (supabase as any).rpc("admin_review_material", {
      _id: id,
      _approve: approve,
    });
  ...
  const reviewEdit = async (id: string, approve: boolean) => {
    const { error } = await (supabase as any).rpc("admin_review_material_edit", {
      _id: id,
      _approve: approve,
      _note: null,
    });
    if (error) return toast.error(error.message);
    toast.success(approve ? "編集を反映しました" : "却下しました");
    load();
  };
    ...
    <div className="space-y-6 mt-4">
      <div>
        <div className="font-bold flex items-center gap-2 mb-2">
          ...
          新規教材の承認 ({pendingMaterials.length})
        </div>
            ...
            <Card key={m.id} className="p-3 flex items-center gap-3 text-sm">
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted-foreground">
                  {m.subject} ・ {m.publisher} ・ 投稿者:{" "}
                  {m.profile?.display_name ?? m.profile?.username ?? "不明"}
                </div>
              </div>
              <Button size="sm" onClick={() => reviewMaterial(m.id, true)}>
                <CheckCircle2 className="h-3.5 w-3.5 mr-1" />
                承認
              </Button>
              <Button size="sm" variant="outline" onClick={() => reviewMaterial(m.id, false)}>
            ...
            return (
              <Card key={e.id} className="p-3 space-y-2 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-xs text-muted-foreground">
                    申請者: {e.profile?.display_name ?? e.profile?.username ?? "不明"}
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  <Button size="sm" onClick={() => reviewEdit(e.id, true)}>
                    <CheckCircle2 className="h-3.5 w-3.5 mr-1" />
                    承認して反映
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => reviewEdit(e.id, false)}>
                    <XCircle className="h-3.5 w-3.5 mr-1" />
                    却下
                  </Button>
                </div>
