...
export function UsersTabImpl() {
  ...
  const seed = async () => {
    try {
      await ensure();
      toast.success("ぽこちゃん管理者を作成/確認しました");
      reload();
    } catch (e: any) {
      toast.error(e.message);
    }
  ...
  const [imp, setImp] = useState<{
    ...
    loading: boolean;
    status: string;
  }>({ open: false, userId: "", loading: false, status: "" });
  ...
  return (
    <div className="space-y-4 mt-4">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            <div className="space-y-2">
              ...
              <Button onClick={submitNew} className="w-full">
                作成
              </Button>
            </div>
          </DialogContent>
          ...
          <tbody>
            {users.map((u) => (
              <UserRow
                ...
                del={del}
                setRole={setRole}
                doImpersonate={startImpersonate}
      ...
      <Dialog open={imp.open} onOpenChange={(o) => !imp.loading && setImp({ ...imp, open: o })}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>ユーザーとしてログイン</DialogTitle>
          </DialogHeader>
          {imp.loading ? (
            ...
            <div className="space-y-3">
              <p className="text-sm">
                選択したユーザーとしてログインします。パスワード不要で切り替えます。
              </p>
              <Button onClick={confirmImpersonate} className="w-full">
...
export function UserRow({
  ...
  const changeRole = async (makeAdmin: boolean) => {
    ...
    try {
      await setRole({ data: { userId: user.id, makeAdmin } });
      toast.success(`${label}に変更しました`);
      onChange();
    } catch (e: any) {
      toast.error(e.message);
    }
  ...
  const isMcjp =
    typeof user.username === "string" && user.username.toLowerCase().startsWith("mcjp_");
  ...
  return (
    <tr className="border-t">
        ...
        {edit ? (
          ...
          <div>
            <div className="font-medium">
              ...
              {isPoko && <span className="ml-2 text-xs text-muted-foreground">(保護)</span>}
            </div>
      ...
      <td className="p-3">
        {locked ? (
          <span className="px-2 py-1 rounded text-xs bg-warning/20 text-warning-foreground inline-flex items-center gap-1">
            <Shield className="h-3 w-3" />
            {isMcjp ? "MCJP固定" : "管理者(固定)"}
          </span>
        ) : (
          <Select
            value={user.isAdmin ? "admin" : "user"}
            onValueChange={(v) => changeRole(v === "admin")}
          >
            <SelectTrigger
              className={`h-8 w-32 ${user.isAdmin ? "border-amber-500/40 bg-amber-500/10" : ""}`}
            >
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
      ...
      <td className="p-3">
        {user.isSuspended ? (
          <span className="px-2 py-1 rounded text-xs bg-destructive/15 text-destructive inline-flex items-center gap-1">
      ...
      <td className="p-3 space-x-1 whitespace-nowrap">
        {locked ? (
          ...
          <>
            ...
            <Button size="sm" variant="outline" onClick={() => doImpersonate(user.id)}>
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="flex gap-2">
              ...
              <Button
                onClick={async () => {
                  const { error } = await (supabase as any).rpc("admin_set_user_xp", {
                    _user_id: userId,
                    _xp: xp,
                  });
