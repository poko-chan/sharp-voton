    ...
    <div className="mt-4 space-y-4">
      <Tabs value={inner} onValueChange={(v) => setInner(v as any)}>
        <TabsList>
          ...
          <TabsTrigger
            value="user"
            className="data-[state=active]:bg-blue-500/10 data-[state=active]:text-blue-600"
          >
            ユーザー別
          </TabsTrigger>
        </TabsList>
...
export function ServiceStopTab() {
  const [rows, setRows] = useState<
    Record<string, { restricted: boolean; message: string; restricted_until: string | null }>
  >({});
  const load = async () => {
  ...
  const save = async (
    key: string,
    patch: Partial<{ restricted: boolean; message: string; restricted_until: string | null }>,
  ) => {
    ...
    <div className="space-y-4 mt-4">
      <Card className="p-4 max-w-3xl space-y-2 border-red-500/30">
        <div className="flex items-center gap-2 text-red-600">
          ...
          <h3 className="font-semibold">サービス別 利用停止（全ユーザー）</h3>
        </div>
        <p className="text-sm text-muted-foreground">
          特定のサービス（タイマー・AIチャット・Voton Classroom
          など）を全ユーザーに対して停止します。該当ページに入ると赤いオーバーレイが表示されます。
        </p>
      </Card>
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted">
            <tr className="text-left">
              ...
              <th className="p-3">停止</th>
              <th className="p-3">メッセージ</th>
              <th className="p-3">解除予定</th>
            </tr>
          </thead>
          <tbody>
            {SERVICES.map((s) => {
                ...
                <tr key={s.key} className="border-t align-top">
                  ...
                  <td className="p-3">
                    <Switch
                      ...
                      onCheckedChange={(v) => save(s.key, { restricted: v })}
                    />
                  </td>
                  <td className="p-3">
                    <Textarea
                      ...
                      onBlur={(e) => save(s.key, { message: e.target.value })}
                  ...
                  <td className="p-3">
                    <Input
                      ...
                      defaultValue={
                        r.restricted_until
                          ? new Date(r.restricted_until).toISOString().slice(0, 16)
                          : ""
                      }
                      onBlur={(e) =>
                        save(s.key, {
                          restricted_until: e.target.value
...
export function UserRestrictionsTab() {
  ...
  const save = async (
    ...
    const cur = restrictions[userId]?.[selectedService] ?? {
      restricted: true,
      message: "",
    ...
    <div className="space-y-4 mt-4">
      <Card className="p-4 max-w-3xl space-y-2 border-blue-500/30">
        <div className="flex items-center gap-2 text-blue-600">
          ...
          <h3 className="font-semibold">サービス別 個別制限</h3>
        </div>
        <p className="text-sm text-muted-foreground">
          特定のユーザー × 特定のサービスへのアクセスを制限します。
        </p>
              ...
              return (
                <tr key={u.id + selectedService} className="border-t align-top">
                  ...
                  <td className="p-3">
                    <Switch
                      checked={restricted}
                      onCheckedChange={(v) => save(u.id, { restricted: v })}
                    />
                  </td>
                  <td className="p-3">
                    <Textarea
                      rows={2}
                      defaultValue={r?.message ?? ""}
                      onBlur={(e) => save(u.id, { message: e.target.value })}
                      placeholder="制限の理由…"
                    />
                  </td>
                  <td className="p-3">
