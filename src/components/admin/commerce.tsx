...
export function OrgsAdminTab() {
  ...
  const create = async () => {
    ...
    const { error } = await (supabase as any)
      .from("organizations")
      .insert({ name, description: desc || null, status: "approved" });
  ...
  const review = async (id: string, approve: boolean) => {
    const { error } = await (supabase as any).rpc("admin_review_organization", {
      _org_id: id,
      _approve: approve,
    });
    if (error) return toast.error(error.message);
        ...
        {apps.map((a: any) => (
          <div key={a.id} className="rounded-lg border p-3 space-y-2 text-sm">
            ...
            <div className="grid gap-1 text-xs text-muted-foreground sm:grid-cols-2">
              <div>
                代表者: {a.rep_last_name} {a.rep_first_name}（{a.rep_last_kana} {a.rep_first_kana}）
              </div>
            ...
            >
              ...
              {openApp === a.id ? "問い合わせを閉じる" : "問い合わせを見る"}
            </Button>
            ...
            <div key={o.id} className="flex items-center gap-2 border rounded p-2 text-sm">
              <div className="flex-1">
                ...
                <div className="text-xs text-muted-foreground">
                  {o.description} ・ 申請者 {o.profile?.display_name ?? o.profile?.username}
                </div>
              </div>
              <Button size="sm" onClick={() => review(o.id, true)}>
                承認
              </Button>
              <Button size="sm" variant="outline" onClick={() => review(o.id, false)}>
                却下
              </Button>
            </div>
          ))}
          ...
          <div key={o.id} className="flex items-center gap-2 border rounded p-2 text-sm">
            <div className="flex-1">
              <div className="font-medium">
                {o.name}{" "}
                <span className="text-[10px] px-1.5 rounded bg-muted ml-1">{o.status}</span>
              </div>
              <div className="text-xs text-muted-foreground">{o.description}</div>
            </div>
            <a href={`/organizations/${o.id}`} className="text-sm underline">
              管理 →
            </a>
          </div>
        ))}
