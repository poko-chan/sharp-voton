...
export function FeedbackTab() {
  ...
  const save = async (id: string, status?: string) => {
    try {
      ...
      toast.success("保存しました。送信者に通知が届きます。");
      reload();
    } catch (e: any) {
      toast.error(e.message);
    }
  ...
  const filtered = items.filter((f) => {
    ...
    if (statusFilter === "active" && (f.status === "resolved" || f.status === "wontfix"))
      return false;
        ...
        <Button
          ...
          onClick={async () => {
            ...
            try {
              ...
              toast.success(`${filtered.length}件をAI用プロンプトとしてコピーしました`);
            } catch {
        ...
        return (
          <Card key={f.id} className="p-4 space-y-2">
            <div className="flex items-center gap-2 flex-wrap text-xs">
              ...
              {f.route && (
                <span className="text-muted-foreground truncate max-w-[200px]">📍 {f.route}</span>
              )}
              {f.user_notified_at && (
                <span className="text-[10px] text-emerald-600">🔔 通知済</span>
              )}
            </div>
            ...
            {!isReview && (
              <>
                <Textarea
                  placeholder="管理者返信メモ（保存すると送信者に通知）"
                  rows={2}
                  defaultValue={f.admin_reply ?? ""}
                  onChange={(e) => setReply((r) => ({ ...r, [f.id]: e.target.value }))}
                />
                <div className="flex gap-2 flex-wrap">
                  <Button size="sm" onClick={() => save(f.id, "in_progress")}>
                    対応中にして通知
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => save(f.id, "resolved")}>
                    解決にして通知
                  </Button>
                  <Button size="sm" variant="ghost" onClick={() => save(f.id)}>
                    返信のみ保存
                  </Button>
                  <Button
                    size="sm"
...
export function AdminThreadDialog({ feedbackId }: { feedbackId: string }) {
  ...
  const load = async () => {
    try {
      setData(await get({ data: { feedbackId } }));
    } catch (e: any) {
      toast.error(e.message);
    }
  };
      ...
      <DialogContent className="max-w-lg p-0 overflow-hidden gap-0">
        ...
        <div className="max-h-[60vh] overflow-auto p-4 space-y-2 bg-muted/20">
          {(data?.messages ?? []).map((m: any) => (
            ...
            >
              <div
                className={`max-w-[80%] px-3 py-2 rounded-2xl text-sm whitespace-pre-wrap break-words ${m.sender_role === "admin" ? "bg-primary text-primary-foreground rounded-br-sm" : "bg-card border rounded-bl-sm"}`}
              >
                <div className="text-[10px] font-semibold opacity-70 mb-1">
                  {m.sender_role === "admin" ? "🛡 管理者(あなた)" : "👤 ユーザー"}
                </div>
                {m.body}
        ...
        <div className="p-2 border-t bg-background flex gap-2">
          ...
          />
          <Button onClick={send} disabled={busy || text.trim().length < 1} size="icon">
