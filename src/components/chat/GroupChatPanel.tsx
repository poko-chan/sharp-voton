...
export function GroupChatPanel({
  ...
  const readCount = (m: GroupMessage) =>
    (readers.data ?? []).filter(
      (r: GroupMember) =>
        ...
        r.last_read_at &&
        new Date(r.last_read_at) >= new Date(m.created_at),
    ).length;
  ...
  const onToggleReaction = async (id: string, emoji: string) => {
    try {
      ...
      toast.error(e.message ?? "リアクションに失敗しました");
    }
  };
  ...
  const startEdit = (m: GroupMessage) => {
    setEditingId(m.id);
  ...
  const saveEdit = async () => {
    ...
    const { error } = await supabase
      .from("chat_group_messages")
      .update({ content: t, edited_at: new Date().toISOString() })
      .eq("id", editingId);
  ...
  const removeMsg = async (id: string) => {
    ...
    const { error } = await supabase
      .from("chat_group_messages")
      .update({ deleted_at: new Date().toISOString(), content: "（削除されました）" })
      .eq("id", id);
    if (error) toast.error(error.message);
    qc.invalidateQueries({ queryKey: ["chat-group-msgs", groupId] });
  };
  ...
  return (
    <>
      ...
      <div
        className={`flex-1 overflow-y-auto p-4 ${prefs.chat_compact ? "space-y-0.5" : "space-y-2"}`}
        style={{ fontSize: `${Math.round(14 * prefs.chat_font_scale)}px` }}
      >
        {(() => {
          let lastDay = "";
          const list = (messages.data ?? []).filter(
            (m) => !query.trim() || m.content?.toLowerCase().includes(query.trim().toLowerCase()),
          );
          return list.flatMap((m) => {
            ...
            const parent = (m as any).reply_to_id
              ? byId.get((m as any).reply_to_id as string)
              : null;
            nodes.push(
              <div key={m.id} className={`flex group ${mine ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[80%] sm:max-w-[70%] space-y-0.5 ${mine ? "items-end" : "items-start"} flex flex-col`}
                >
                  {!mine && (
                  ...
                  <Card
                    className={`px-3 py-2 ${mine ? "!bg-primary !text-primary-foreground border-primary" : ""} ${isDeleted ? "opacity-60 italic" : ""}`}
                  >
                    {parent && (
                      <div
                        className={`mb-1 border-l-2 pl-2 text-[11px] line-clamp-2 ${mine ? "border-primary-foreground/60 opacity-80" : "border-muted-foreground/40 text-muted-foreground"}`}
                      >
      ...
      {replyTo && (
        <div className="border-t px-3 py-2 flex items-center gap-2 bg-muted/50">
          ...
          <span className="text-xs text-muted-foreground truncate flex-1">
            {nameOf(replyTo.sender_id)} に返信: {replyTo.content}
          </span>
          <Button
            size="icon"
            variant="ghost"
