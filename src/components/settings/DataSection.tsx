...
export function DataSection() {
  ...
  const csv = async () => {
    ...
    const { data } = await supabase
      .from("study_logs")
      .select("date,duration_minutes,memo,tag,subject_id")
      .eq("user_id", user.id)
      .order("date");
    const rows = [
      "date,duration_minutes,memo,tag,subject_id",
      ...(data ?? []).map(
        (r: any) =>
          `${r.date},${r.duration_minutes},"${(r.memo ?? "").replace(/"/g, '""')}",${r.tag ?? ""},${r.subject_id ?? ""}`,
      ),
    ];
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          <Button onClick={csv} disabled={busy} variant="outline">
            CSVダウンロード
          </Button>
          <Button onClick={ics} disabled={busy} variant="outline">
            .icsダウンロード
          </Button>
        </div>
      </Card>
