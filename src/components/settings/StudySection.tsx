...
function TownSettings() {
  ...
  const canCreate =
    active.length === 0 || active.some((t) => (t.max_stage_reached ?? 0) >= MAX_STAGE);
  ...
  return (
    <Card className="p-6 space-y-4">
      <div className="flex items-center gap-2 font-semibold">
        <Sparkles className="h-4 w-4 text-primary" /> 町（目標）
      </div>
