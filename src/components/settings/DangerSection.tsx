...
export function DangerSection() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setCounts({
        logs: results[0].count ?? 0,
        goals: results[1].count ?? 0,
        questions: results[2].count ?? 0,
        plans: results[3].count ?? 0,
        subjects: results[4].count ?? 0,
        events: results[5].count ?? 0,
        ai: results[6].count ?? 0,
        tutor: results[7].count ?? 0,
      });
    })();
  }, [user]);
  ...
  return (
    <div className="space-y-6">
      <SectionHeading
        title="危険な操作"
        desc="元に戻せない操作です。十分に注意して行ってください"
      />
