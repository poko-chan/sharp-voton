    ...
    <Card className="p-4 space-y-2">
      ...
      {(post.minutes || post.subject) && (
        <div className="flex gap-2 text-[11px]">
          ...
          {post.minutes ? (
            <span className="px-2 py-0.5 rounded bg-primary/10 text-primary">{post.minutes}分</span>
          ) : null}
        </div>
      )}
      <div className="flex items-center gap-3 pt-1">
        <button
          ...
          className={`flex items-center gap-1 text-xs ${like?.mine ? "text-red-500" : "text-muted-foreground"}`}
        >
          <Heart className={`h-4 w-4 ${like?.mine ? "fill-current" : ""}`} />
          {like?.count ?? 0}
        </button>
        <button
