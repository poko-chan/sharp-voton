...
export function AmbientSound() {
  ...
  const [customs, setCustoms] = useState<
    { id: string; label: string; url?: string; path?: string }[]
  >(() => {
    try {
      return JSON.parse(localStorage.getItem("lovable.ambient.customs.v1") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const persistCustoms = (next: typeof customs) => {
    setCustoms(next);
    try {
      localStorage.setItem("lovable.ambient.customs.v1", JSON.stringify(next));
    } catch {}
  };
  ...
  const playCustom = async (c: { id: string; url?: string; path?: string }) => {
    ...
    if (c.path) {
      const { data, error } = await supabase.storage
        .from("ambient-audio")
        .createSignedUrl(c.path, 60 * 60 * 6);
  ...
  const removeCustom = async (c: (typeof customs)[number]) => {
    if (c.path) {
      try {
        await supabase.storage.from("ambient-audio").remove([c.path]);
      } catch {}
    }
    ...
    <>
      ...
      >
        {preset === "off" ? (
          ...
          <Volume2 className="h-5 w-5 drop-shadow animate-pulse" />
        )}
      </button>
      {open && (
        <div className="fixed bottom-36 right-4 z-[60] w-72 rounded-2xl border border-white/20 bg-popover/95 backdrop-blur-2xl p-4 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.4)] space-y-3 ring-1 ring-white/10">
          ...
          <div className="pt-2 border-t border-border/60 space-y-1.5">
            ...
            <div className="max-h-32 overflow-auto space-y-1">
              {customs.length === 0 && (
                <div className="text-[10px] text-muted-foreground">まだ登録されていません</div>
              )}
              {customs.map((c) => (
                <div
                  key={c.id}
                  className={`flex items-center gap-1.5 text-xs rounded border px-2 py-1 ${playingCustomId === c.id ? "bg-primary/15 border-primary" : ""}`}
                >
                  <button
                    className="flex-1 text-left truncate"
                    onClick={() => (playingCustomId === c.id ? stop() : playCustom(c))}
                  >
                    {playingCustomId === c.id ? "⏸" : "▶"} {c.label}{" "}
                    {c.path && <span className="text-[9px] text-muted-foreground">📁</span>}
                  </button>
                  <button
