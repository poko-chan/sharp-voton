...
export function StudySession({ deckName, cards, shuffled = false, onExit }: Props) {
  ...
  const [results, setResults] = useState<Record<Grade, number>>({
    again: 0,
    hard: 0,
    good: 0,
    easy: 0,
    ...
    return (
      <Card className="p-8 text-center space-y-4">
        ...
        <div className="flex justify-center gap-2 flex-wrap">
          {missed.length > 0 && (
            <Button variant="outline" onClick={retryMissed}>
              間違いだけ復習
            </Button>
          )}
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-6 sm:p-8 text-center space-y-4 min-h-[240px] flex flex-col justify-center">
        <div className="text-xl sm:text-2xl font-medium whitespace-pre-wrap break-words">
          {current.front}
        </div>
        {showBack && (
        ...
        {!showBack ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            <Button disabled={busy} onClick={() => handleGrade("good")}>
              ふつう
            </Button>
            <Button
