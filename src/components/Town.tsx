    ...
    return (
      <Card className="p-6 space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          で目標を入力して街を作りましょう。学習内容が人口・GDP・CO2として街に反映されます。
        </p>
      </Card>
    );
  }
      ...
      {towns.length > 1 && (
        <div className="flex items-center justify-between px-4 py-2 border-b bg-muted/30 text-xs">
          <Button
            ...
            variant="ghost"
            className="h-7 px-2"
            onClick={() => setActiveIdx((i) => (i - 1 + towns.length) % towns.length)}
          >
            <ArrowLeft className="h-3 w-3" />
          </Button>
          <span className="text-muted-foreground">
            {activeIdx + 1} / {towns.length}
          </span>
          <Button
            size="sm"
            variant="ghost"
            className="h-7 px-2"
            onClick={() => setActiveIdx((i) => (i + 1) % towns.length)}
          >
  ...
  const load = async () => {
    ...
    setStudy({
      ...
      avgScore: grades.length
        ? Math.round(grades.reduce((s: number, g: any) => s + (g.score ?? 0), 0) / grades.length)
        : 0,
...
function MetricTile({
  ...
  return (
    <div className="p-3 rounded-xl border bg-card/60 flex items-start gap-2.5">
      <div className={`h-8 w-8 rounded-lg grid place-items-center shrink-0 ${cls}`}>
...
function TownCard({ town, onUpdate }: { town: TownRow; onUpdate: () => void }) {
  ...
  const buildAt = async (gx: number, gz: number, kind?: string) => {
    ...
    if (buildings.some((b) => b.gx === gx && b.gz === gz))
      return toast.error("この区画にはすでに建物があります");
    if (town.stage < def.minStage)
      return toast.error(`ステージ ${def.minStage} 以上で建設できます`);
  ...
  const demolish = async () => {
    ...
    setBusy(true);
    const { error } = await (supabase as any).rpc("town_demolish", {
      _building_id: selectedBuilding.id,
      _refund: refund,
    });
    setBusy(false);
    if (error) return toast.error(error.message);
          ...
          {metrics ? (
            <>
              <MetricTile
                icon={Users}
                label="人口"
                value={fmtNum(metrics.population)}
                sub={`成長率 ${metrics.growthPct > 0 ? "+" : ""}${metrics.growthPct}%`}
                tone="primary"
              />
              <MetricTile
                icon={TrendingUp}
                label="GDP"
                value={`${fmtNum(metrics.gdp)} 百万円`}
                sub={`一人当たり ${metrics.gdpPerCapita} 万円`}
                tone="emerald"
              />
              <MetricTile
                icon={Leaf}
                label="CO2排出"
                value={`${fmtNum(metrics.co2)} t/年`}
                sub={`環境スコア ${metrics.green}`}
                tone="amber"
              />
              <MetricTile
                icon={Smile}
                ...
                sub={`${stageName(town.stage)} ・ 建物 ${buildings.length}`}
                tone="primary"
              />
            </>
          ) : (
          ...
          <Tabs defaultValue={localPrefs.town_default_tab} className="mt-2">
                ...
                <>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    ...
                    <MetricTile
                      ...
                      label="CO2排出"
                      value={`${fmtNum(metrics.co2)} t/年`}
                      sub={`一人当たり ${metrics.co2PerCapita} kg`}
                      tone="amber"
                    />
                    <MetricTile
                      icon={Smile}
                      label="幸福度 / 環境"
                      value={`${metrics.happiness} / ${metrics.green}`}
                      tone="primary"
                    />
                  </div>
                  <div className="rounded-lg border p-3 text-xs space-y-1 bg-muted/20">
                    <div className="font-semibold text-[11px] text-muted-foreground">
                      この指標を決めている勉強データ（直近30日）
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-1">
                      <span>
                        学習時間: <b>{study.minutes30}分</b>
                      </span>
                      <span>
                        活動日数: <b>{study.activeDays30}日</b>
                      </span>
                      <span>
                        連続日数: <b>{study.streak}日</b>
                      </span>
                      <span>
                        採点平均: <b>{study.avgScore}点</b>
                      </span>
                      <span>
                        学習教科数: <b>{study.subjects}</b>
                      </span>
                      <span>
                        達成した目標: <b>{study.goalsDone}</b>
                      </span>
                    </div>
              ...
              {POLICY_DEFS.map((p) => {
                ...
                const preview = study
                  ? computeMetrics(
                      ...
                      study,
                      buildings,
                      on ? policies.filter((k) => k !== p.key) : [...policies, p.key],
                    )
                  : null;
                return (
                  <div
                    key={p.key}
                    className={`rounded-lg border p-3 flex items-start gap-3 ${unlocked ? "" : "opacity-60"}`}
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {unlocked && preview && metrics && (
                        <div className="text-[11px] mt-1 tabular-nums text-muted-foreground">
                          {on ? "OFFにすると" : "ONにすると"} 人口{" "}
                          {sign(preview.population - metrics.population)} ・ GDP{" "}
                          {sign(preview.gdp - metrics.gdp)} ・ CO2 {sign(preview.co2 - metrics.co2)}
                        </div>
                      )}
                    </div>
                    <Switch
                      checked={on}
                      disabled={!unlocked}
                      onCheckedChange={(v) => togglePolicy(p.key, v)}
                    />
                  </div>
                );
              })}
            ...
            <TabsContent value="build" className="mt-3 space-y-3">
              <div className="rounded-lg border bg-muted/20 p-2.5 flex flex-wrap items-center gap-2 text-xs sticky top-0 z-10 backdrop-blur">
                ...
                >
                  <Hammer className="h-3.5 w-3.5 mr-1" />
                  建設モード{buildMode ? "ON" : "OFF"}
                </Button>
                <span className="font-medium">
                  {picked
                    ? `${buildDef(picked)?.emoji} ${buildDef(picked)?.label} を選択中`
                    : "建物を選んでください"}
                </span>
                {picked && (
                  <Button
                ...
                <span className="text-muted-foreground ml-auto">
                  ...
                  {selectedBuilding &&
                    ` ・ ${buildDef(selectedBuilding.kind)?.label ?? selectedBuilding.kind}`}
                </span>
              </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`ml-auto text-xs tabular-nums flex items-center gap-0.5 ${poor ? "text-destructive" : "text-amber-600"}`}
                          >
                    ...
                    selectedBuilding ? (
                      <div className="rounded-lg border p-3 space-y-1">
                        <div className="text-base font-bold">
                          {buildDef(selectedBuilding.kind)?.emoji}{" "}
                          {buildDef(selectedBuilding.kind)?.label}
                        </div>
                        <div className="text-xs text-muted-foreground">
                          位置 ({selected[0]}, {selected[1]}) ・ Lv{selectedBuilding.level}
                        </div>
                        <div className="text-xs tabular-nums">
                          人口 {sign(buildDef(selectedBuilding.kind)?.pop ?? 0)} ・ GDP{" "}
                          {sign(buildDef(selectedBuilding.kind)?.gdp ?? 0)} ・ CO2{" "}
                          {sign(buildDef(selectedBuilding.kind)?.co2 ?? 0)}
                        </div>
                        <Button
                          size="sm"
                          variant="outline"
                      ...
                      <div className="rounded-lg border p-3 text-xs text-muted-foreground">
                        空き区画 ({selected[0]}, {selected[1]}
                        )。「建設」タブから建物を選んで建てられます。
                      </div>
                    )
                  ) : (
                    <div className="text-xs text-muted-foreground">
                      地図か3Dの区画をクリックすると詳細が表示されます。
                    </div>
                  )}
            ...
            <TabsContent value="info" className="mt-3 space-y-3 text-sm">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                ...
                <Info
                  ...
                  value={
                    town.last_judged_at
                      ? new Date(town.last_judged_at).toLocaleDateString("ja-JP")
                      : "未判定"
                  }
                />
              </div>
              ...
              {lastResult && (
                <div className="rounded border bg-primary/5 border-primary/30 p-3 space-y-1">
                  <div className="text-xs font-semibold flex items-center gap-2">
                    <Sparkles className="h-3 w-3" /> 判定結果 (Δ{" "}
                    {lastResult.delta > 0 ? `+${lastResult.delta}` : lastResult.delta})
                  </div>
                  ...
                  {history.map((h) => (
                    <div key={h.id} className="border-b last:border-0 py-1">
                      <div className="flex justify-between">
                        ...
                        >
                          Δ {h.delta > 0 ? `+${h.delta}` : h.delta} ({h.stage_before}→
                          {h.stage_after})
                        </span>
                      </div>
