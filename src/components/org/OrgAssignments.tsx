...
export function OrgAssignments({ orgId, canAdmin, packs, members }: Props) {
  ...
  const showProgress = async (id: string) => {
    ...
    const { data, error } = await (supabase as any).rpc("org_assignment_progress", {
      _assignment: id,
    });
    if (error) return toast.error(error.message);
  ...
  return (
    <div className="space-y-3">
        ...
        <Card className="p-4 space-y-3">
          ...
          {creating && (
            <div className="space-y-2">
              <Input
                placeholder="課題タイトル（例: 今週の英単語）"
                value={title}
                onChange={(e) => setTitle(e.target.value)}
              />
              <Select value={packId} onValueChange={setPackId}>
                <SelectTrigger className="h-9">
                  <SelectValue placeholder="組織の問題集を選択" />
                </SelectTrigger>
                <SelectContent>
                  {packs.map((p) => (
                ...
                <div className="max-h-44 overflow-auto border rounded p-2 space-y-1">
                  {members.map((m: any) => (
                    <label key={m.user_id} className="flex items-center gap-2 text-sm">
                      <input
                        type="checkbox"
                        checked={targets.includes(m.user_id)}
                        onChange={(e) =>
                          setTargets((t) =>
                            e.target.checked ? [...t, m.user_id] : t.filter((x) => x !== m.user_id),
                          )
                        }
                      />
      ...
      {rows.length === 0 && (
        <Card className="p-6 text-sm text-muted-foreground">課題はまだありません</Card>
      )}
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex-1 min-w-[180px]">
                <div className="font-medium flex items-center gap-2">
                  {a.title}
                  {a.required && (
                    <span className="text-[10px] px-1.5 rounded bg-primary/15 text-primary">
                      必須
                    </span>
                  )}
                  {!a.assign_all && (
                    <span className="text-[10px] px-1.5 rounded bg-muted">指名</span>
                  )}
                </div>
                <div className="text-[11px] text-muted-foreground flex items-center gap-1">
                  ...
                  {a.due_at && (
                    <span className={overdue ? "text-destructive" : ""}>
                      <CalendarClock className="h-3 w-3 inline mx-1" />
                      {jstFormat(a.due_at)}まで
                    </span>
                  )}
                </div>
                {a.instructions && (
                  <div className="text-xs mt-1 whitespace-pre-wrap">{a.instructions}</div>
                )}
              </div>
              <Link
                to="/makron/pack/$packId"
                params={{ packId: a.pack_id }}
              ...
              {canAdmin && (
                <>
                  <Button size="sm" variant="outline" onClick={() => showProgress(a.id)}>
              ...
              <div className="border-t pt-2 space-y-1">
                {(progress[a.id] ?? []).map((p: any) => (
                  <div key={p.user_id} className="flex flex-wrap items-center gap-2 text-xs">
                    ...
                    <span className={p.done ? "text-emerald-600" : "text-muted-foreground"}>
                      {p.done ? "完了" : "未着手"}
                    </span>
                    <span>最高 {p.best_score ?? 0}%</span>
                    <span className="text-muted-foreground">
                      {p.attempts ?? 0}回 / {p.last_at ? jstFormat(p.last_at) : "—"}
                    </span>
                  </div>
                ))}
                {(progress[a.id] ?? []).length === 0 && (
                  <div className="text-xs text-muted-foreground">対象者がいません</div>
                )}
              </div>
            )}
          </Card>
