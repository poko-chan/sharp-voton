...
export function splitSections(qs: any[]) {
  const secs: { title: string; desc?: string; items: any[] }[] = [
    { title: "", desc: "", items: [] },
  ];
  for (const q of qs ?? []) {
...
export function OrgSurveys({ orgId, ctx }: { orgId: string; ctx: any }) {
  ...
  const create = async () => {
    ...
    if (qs.some((q) => q.type !== "section" && !q.label.trim()))
      return toast.error("質問文が空のものがあります");
    const { error } = await (supabase as any).from("org_surveys").insert({
      organization_id: orgId,
      group_id: scope === "org" ? null : scope,
      title: title.trim(),
    ...
    await (supabase as any).rpc("org_notify_members", {
      _org: orgId,
      _group: scope === "org" ? null : scope,
      _app: "surveys",
      _title: `アンケート: ${title.trim()}`,
      _body: desc || null,
    });
  ...
  const renderInput = (q: any) => {
    ...
    switch (q.type) {
          ...
          <div className="flex items-center gap-4 pt-1">
            ...
            {[1, 2, 3, 4, 5].map((n) => (
              ...
              >
                <span>{n}</span>
                <span
                  className={`h-4 w-4 rounded-full border-2 ${v === n ? "bg-primary border-primary" : "border-muted-foreground/40"}`}
                />
              </button>
            ))}
          ...
          <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map((n) => (
              <button key={n} type="button" onClick={() => set(n)}>
                <Star
                  className={`h-6 w-6 ${(v ?? 0) >= n ? "fill-amber-400 text-amber-400" : "text-muted-foreground/40"}`}
                />
              </button>
            ))}
          </div>
      ...
      default:
        return (
          <Input placeholder="回答を入力" value={v ?? ""} onChange={(e) => set(e.target.value)} />
        );
    }
  };
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-3">
        ...
        <Card className="overflow-hidden border-t-[6px] border-t-primary p-6 space-y-3">
          <Input
            className="!text-2xl font-bold h-12 border-0 border-b rounded-none px-0 focus-visible:ring-0"
            ...
            <Card key={q.id} className="p-5 space-y-2 border-l-4 border-l-primary">
              <div className="flex items-center gap-2">
                <div className="text-[11px] font-semibold text-primary">
                  セクション {qs.slice(0, i + 1).filter((x) => x.type === "section").length + 1}
                </div>
                <button
                  className="ml-auto text-muted-foreground hover:text-destructive"
                  onClick={() => setQs((a) => a.filter((_, idx) => idx !== i))}
                >
              ...
              <Input
                ...
                value={q.desc ?? ""}
                onChange={(e) => setQ(i, { desc: e.target.value })}
              />
            </Card>
          ) : (
            <Card key={q.id} className="p-5 space-y-3 hover:border-primary/50 transition">
              <div className="flex justify-center -mt-2 text-muted-foreground/40">
                <GripVertical className="h-4 w-4 rotate-90" />
              </div>
              <div className="flex flex-col sm:flex-row gap-3">
                <Input
                  className="flex-1 border-0 border-b rounded-none px-0 focus-visible:ring-0 bg-muted/30"
                  placeholder={`質問 ${i + 1}`}
                  value={q.label}
                  onChange={(e) => setQ(i, { label: e.target.value })}
                />
                <Select
                  ...
                  onValueChange={(t) =>
                    setQ(i, {
                      type: t,
                      options:
                        CHOICE.includes(t) && !(q.options ?? []).length ? ["選択肢 1"] : q.options,
                    })
                  }
                >
                ...
                <div className="space-y-2">
                  {(q.options ?? []).map((o: string, oi: number) => (
                    <div key={oi} className="flex items-center gap-2">
                      <span
                        className={`h-4 w-4 border-2 border-muted-foreground/40 ${q.type === "multi" ? "rounded-[3px]" : "rounded-full"}`}
                      />
                      <Input
                        className="flex-1 border-0 border-b rounded-none px-0 h-8 focus-visible:ring-0"
                        value={o}
                        onChange={(e) =>
                          setQ(i, {
                            options: q.options.map((x: string, xi: number) =>
                      ...
                      <button
                        ...
                        onClick={() =>
                          setQ(i, {
                            options: q.options.filter((_: string, xi: number) => xi !== oi),
                          })
                        }
                      >
                        <Trash2 className="h-4 w-4" />
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setQ(i, {
                        options: [...(q.options ?? []), `選択肢 ${(q.options?.length ?? 0) + 1}`],
                      })
                    }
                  >
              ...
              {q.type === "yesno" && (
                <div className="text-xs text-muted-foreground">選択肢：はい / いいえ</div>
              )}
                  ...
                  {(q.type === "yesno" ? ["はい", "いいえ"] : (q.options ?? []))
                    ...
                    .map((o: string) => (
                      <div key={o} className="flex items-center gap-2">
                        ...
                        <Select
                          value={(q.goto ?? {})[o] ?? "next"}
                          onValueChange={(v) => setQ(i, { goto: { ...(q.goto ?? {}), [o]: v } })}
                        >
              ...
              <div className="flex items-center justify-end gap-3 border-t pt-3">
                ...
                <button
                  className="text-muted-foreground hover:text-destructive"
                  title="削除"
                  onClick={() => setQs((a) => (a.length > 1 ? a.filter((_, idx) => idx !== i) : a))}
                >
                  <Trash2 className="h-4 w-4" />
                </button>
                <label className="text-xs flex items-center gap-2 border-l pl-3">
                  必須
                  <Switch checked={q.required} onCheckedChange={(v) => setQ(i, { required: v })} />
                </label>
              </div>
            </Card>
          ),
        ...
        <div className="flex gap-2 sticky bottom-4">
          <Button
            variant="outline"
            onClick={() => setQs((a) => [...a, { ...newQ(), type: "text", options: [] }])}
          >
            <Plus className="h-4 w-4 mr-1" />
            質問を追加
          </Button>
          <Button variant="outline" onClick={() => setQs((a) => [...a, newSection()])}>
            <Plus className="h-4 w-4 mr-1" />
            セクションを追加
          </Button>
  ...
  if (view === "answer" && current) {
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-3">
        ...
        {current.anonymous && (
          <div className="text-xs text-muted-foreground px-1">この回答は匿名で集計されます。</div>
        )}
        {secs.length > 1 && (
          <div className="text-xs text-muted-foreground px-1">
            セクション {secIdx + 1} / {secs.length}
            {sec.title ? `：${sec.title}` : ""}
          </div>
        )}
        {sec.desc && (
          <Card className="p-4 text-sm text-muted-foreground whitespace-pre-wrap">{sec.desc}</Card>
        )}
        {sec.items.map((q: any) => (
        ...
        <div className="flex gap-2">
          {secPath.length > 0 && (
            <Button
              variant="outline"
              onClick={() => {
                setSecIdx(secPath[secPath.length - 1]);
                setSecPath((p) => p.slice(0, -1));
              }}
            >
              戻る
            </Button>
                  ...
                  {isChoice ? (
                    ...
                    <div className="text-sm">
                      ...
                      <span className="font-bold text-lg">
                        {vals.length
                          ? (
                              vals.reduce((a: number, b: any) => a + Number(b), 0) / vals.length
                            ).toFixed(2)
                          : "—"}
                      </span>
                      （{vals.length}件）
                    ...
                    <div className="space-y-1">
                      {vals.length === 0 && (
                        <div className="text-xs text-muted-foreground">回答なし</div>
                      )}
                      {vals.map((v: any, i: number) => (
                        <div key={i} className="text-sm rounded bg-muted/50 px-3 py-1.5">
                          {String(v)}
                        </div>
                      ))}
                    </div>
          ...
          results.map((r: any) => (
            <Card key={r.id} className="p-4 space-y-1 text-sm">
              <div className="text-[11px] text-muted-foreground">
                {current.anonymous ? "匿名" : nameOf(profiles[r.user_id])}
              </div>
              {(current.questions ?? [])
                .filter((q: any) => q.type !== "section")
                .map((q: any) => (
        ...
        {rows.map((s) => (
          <Card key={s.id} className="p-0 overflow-hidden flex flex-col">
            ...
            <div className="p-4 space-y-2 flex-1 flex flex-col">
              ...
              <div className="text-[11px] text-muted-foreground">
                {s.group_id
                  ? (ctx.groups.find((g: any) => g.id === s.group_id)?.name ?? "グループ")
                  : "組織全体"}
                {s.anonymous && " ・匿名"} ・ 質問
                {(s.questions ?? []).filter((q: any) => q.type !== "section").length}問
              </div>
              {myResponses[s.id] && (
              ...
              <div className="flex gap-2 pt-2 mt-auto">
                <Button
                  ...
                  onClick={() => {
                    setCurrent(s);
                    setAnswers(myResponses[s.id]?.answers ?? {});
                    setSecIdx(0);
                    setSecPath([]);
                    setView("answer");
                  }}
                ...
                {(s.created_by === user?.id || ctx.canAdmin) && (
                  <>
                    <Button size="sm" variant="outline" onClick={() => showResults(s)}>
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        await (supabase as any).from("org_surveys").delete().eq("id", s.id);
                        load();
                      }}
                    >
                      <Trash2 className="h-4 w-4" />
