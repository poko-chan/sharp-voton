...
const fileName = (u: string) =>
  decodeURIComponent(u.split("/").pop() ?? "file").replace(/^[0-9a-f-]{36}-/i, "");
...
export function OrgPosts({ orgId, ctx }: { orgId: string; ctx: any }) {
  ...
  useEffect(() => {
    try {
      setPins(JSON.parse(localStorage.getItem(PIN_KEY(orgId)) ?? "[]"));
    } catch {
      setPins([]);
    }
  }, [orgId]);
  ...
  const load = async () => {
    ...
    if (ids.length) {
      ...
      setLikes(lmap);
      setComments(cmap);
      const more = await loadOrgProfiles(
        orgId,
        (cm ?? []).map((c: any) => c.user_id),
      );
  ...
  const upload = async (files: FileList | null) => {
    ...
    for (const f of Array.from(files)) {
      if (f.size > 25 * 1024 * 1024) {
        toast.error(`${f.name} は25MBを超えています`);
        continue;
      }
  ...
  const create = async () => {
    ...
    await (supabase as any).rpc("org_notify_members", {
      ...
      _app: "posts",
      _title: `新しい投稿: ${title.trim()}`,
      _body: body.slice(0, 80),
    });
  ...
  const countOf = (key: string) =>
    posts.filter((p) => (key === "all" ? true : key === "org" ? !p.group_id : p.group_id === key))
      .length;

  const channels = useMemo(() => {
    const base = [
      { id: "all", name: "すべての投稿", icon: "all" },
      { id: "org", name: "組織全体", icon: "org" },
    ];
  ...
  const visible = posts.filter((p) =>
    channel === "all" ? true : channel === "org" ? !p.group_id : p.group_id === channel,
  );

  const ChannelBtn = ({ id, name, kind }: { id: string; name: string; kind: string }) => (
    <div
      className={`group flex items-center gap-1 rounded-lg px-2 py-1.5 text-sm cursor-pointer transition
      ${channel === id ? "bg-primary/12 text-primary font-medium" : "hover:bg-muted"}`}
    ...
    >
      ...
      {kind === "group" && (
        <button
          ...
          title={pins.includes(id) ? "ピン解除" : "ピン止め"}
          onClick={(e) => {
            e.stopPropagation();
            togglePin(id);
          }}
    ...
    <div className="grid lg:grid-cols-[220px_1fr] gap-5">
      ...
      <aside className="space-y-3 lg:sticky lg:top-4 self-start">
        <Card className="p-2 space-y-0.5">
          ...
          <div className="px-1 pb-1 space-y-1">
            ...
            <div className="flex gap-1">
              {(
                ...
                >
                  {k === "name" && <ArrowDownUp className="h-2.5 w-2.5 inline mr-0.5" />}
                  {l}
                </button>
              ))}
            </div>
          ...
          {channels.base.map((c) => (
            <ChannelBtn key={c.id} id={c.id} name={c.name} kind={c.icon} />
          ))}
          {channels.pinned.length > 0 && (
            <>
              <div className="px-2 pt-2 pb-1 text-[10px] font-semibold tracking-wide text-muted-foreground uppercase flex items-center gap-1">
                <Pin className="h-3 w-3" />
                ピン止め
              </div>
              {channels.pinned.map((g: any) => (
                <ChannelBtn key={g.id} id={g.id} name={g.name} kind="group" />
              ))}
            </>
          )}
        ...
        <Button
          className="w-full"
          onClick={() => {
            setScope(channel === "all" ? "org" : channel);
            setCreating(true);
          }}
        >
              ...
              {images.map((u) => (
                <span key={u} className="relative">
                  {isImage(u) ? (
                    ...
                    <span className="inline-flex items-center gap-1 h-14 px-2 rounded border text-[11px] max-w-[140px] truncate">
                  ...
                  <button
                    ...
                    title="削除"
                    onClick={() => setImages((i) => i.filter((x) => x !== u))}
                  >
                    <X className="h-3 w-3" />
                  </button>
                </span>
        ...
        {visible.map((p) => (
          <Card key={p.id} className="p-4 space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-muted-foreground">
                  {nameOf(profiles[p.author_id])} ・{" "}
                  {new Date(p.created_at).toLocaleString("ja-JP")}
                  <span className="ml-2 px-1.5 rounded bg-sky-500/15 text-sky-600">
                    {p.group_id ? (groupName(p.group_id) ?? "グループ") : "組織全体"}
                  </span>
                </div>
              </div>
              {(p.author_id === user?.id || ctx.canAdmin) && (
                <div className="flex gap-1">
                  <Button size="sm" variant="ghost" onClick={() => startEdit(p)} title="編集">
                ...
                {p.images
                  ...
                  .map((u: string) => (
                    ...
                    >
                      ...
                      <span className="truncate flex-1">{fileName(u)}</span>
            ...
            <div className="flex gap-4 text-sm pt-1">
              ...
              >
                <Heart
                  className={`h-4 w-4 ${(likes[p.id] ?? []).includes(user?.id ?? "") ? "fill-current text-rose-500" : ""}`}
                />
                {(likes[p.id] ?? []).length}
              </button>
              <button
            ...
            {openId === p.id && (
              <div className="space-y-2 pt-2 border-t">
                  ...
                  <div key={c.id} className="text-sm flex gap-2">
                    ...
                    {c.user_id === user?.id && (
                      <button
                        ...
                        onClick={async () => {
                          await (supabase as any).from("org_post_comments").delete().eq("id", c.id);
                          load();
                        }}
                      >
                        削除
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <Button size="sm" onClick={() => comment(p.id)}>
