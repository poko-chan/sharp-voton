...
export function OrgRoster({ orgId }: { orgId: string }) {
  ...
  const copyPrev = async () => {
    ...
    const old = await loadOrgYearValues(
      orgId,
      prev,
      members.map((m) => m.user_id),
    );
  ...
  const list = useMemo(
    () => members.filter((m) => !q.trim() || nameOf(profiles[m.user_id]).includes(q.trim())),
    [members, profiles, q],
  );
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        教師以上のみが編集できます。年度を切り替えると、その年度の内容を編集できます。
      </p>
      ...
      {fields.length === 0 && (
        <Card className="p-6 text-sm text-muted-foreground">
          先に「プロフィール項目」で学年・クラスなどの項目を作成してください。
        </Card>
      )}
        ...
        <Card key={m.user_id} className="p-3 flex flex-wrap items-center gap-2">
          ...
          {fields.map((f) => (
            <div key={f.id} className="flex items-center gap-1">
              ...
              {f.type === "select" ? (
                <Select
                  value={values[m.user_id]?.[f.key] ?? ""}
                  onValueChange={(v) => set(m.user_id, f.key, v)}
                >
                ...
                <Input
                  ...
                  value={values[m.user_id]?.[f.key] ?? ""}
                  onChange={(e) => set(m.user_id, f.key, e.target.value)}
                />
              )}
            </div>
          ))}
