...
function QR({ value }: { value: string }) {
  ...
  useEffect(() => {
    if (value)
      QRCode.toDataURL(value, { margin: 1, width: 140 })
        .then(setUrl)
        .catch(() => {});
  }, [value]);
    ...
    const { error } = await (supabase as any).from("org_digital_ids").upsert(
      {
        ...
        barcode_value: e.barcode_value ?? null,
    ...
    <Tabs defaultValue="mine">
      ...
      <TabsContent value="mine">
        {mine ? (
          ...
          <Card className="p-6 text-sm text-muted-foreground">自分の証はまだありません</Card>
        )}
      </TabsContent>
          ...
          return (
            <Card key={m.user_id} className="p-3 space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Input
                  className="w-36 h-8"
                  placeholder="所属（例: 1年C組）"
                  defaultValue={d.affiliation ?? ""}
                  onChange={(ev) => set({ affiliation: ev.target.value })}
                />
                <Input
                  className="w-32 h-8"
                  placeholder="学籍番号"
                  defaultValue={d.id_number ?? ""}
                  onChange={(ev) => set({ id_number: ev.target.value })}
                />
                <Input
                  className="w-40 h-8"
                  placeholder="バーコード値"
                  defaultValue={d.barcode_value ?? ""}
                  onChange={(ev) => set({ barcode_value: ev.target.value })}
                />
                <Input
                  type="date"
                  className="w-40 h-8"
                  defaultValue={d.valid_until ?? ""}
                  onChange={(ev) => set({ valid_until: ev.target.value })}
                />
                <Button size="sm" onClick={() => save(m.user_id)}>
