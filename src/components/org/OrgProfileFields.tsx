...
const slug = (label: string, i: number) =>
  (label.replace(/[^\w]/g, "").toLowerCase() || `field${i + 1}`) +
  "_" +
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        表示名以外の項目は組織が自由に決められます（例:
        塾なら「教室番号」、学校なら「学年」「クラス」）。
      ...
      <Card className="p-4 flex flex-wrap items-center gap-2">
        ...
        <span className="text-[11px] text-muted-foreground">
          年度ごとに学年・クラスを持てます（進級しても過去の記録が残ります）。
        </span>
      </Card>
      ...
      {fields.length === 0 && (
        <Card className="p-8 text-center text-sm text-muted-foreground">項目はまだありません</Card>
      )}
        ...
        <Card key={f.id} className="p-4 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Input
              className="w-44 h-8"
              value={f.label}
              onChange={(e) => patch(f, { label: e.target.value })}
            />
            ...
            <label className="flex items-center gap-1 text-xs">
              必須
              <Switch checked={f.required} onCheckedChange={(v) => patch(f, { required: v })} />
            </label>
            <label className="flex items-center gap-1 text-xs">
              教師以上のみ編集
              <Switch checked={f.staff_only} onCheckedChange={(v) => patch(f, { staff_only: v })} />
            </label>
            <label className="flex items-center gap-1 text-xs">
              年度ごと
              <Switch checked={f.yearly} onCheckedChange={(v) => patch(f, { yearly: v })} />
            </label>
            <Button
              size="sm"
              variant="ghost"
          ...
          {f.type === "select" && (
            <div className="space-y-1 pl-6">
              {f.options.map((o, i) => (
                <div key={i} className="flex gap-2">
                  <Input
                    ...
                    value={o}
                    onChange={(e) =>
                      patch(f, { options: f.options.map((x, j) => (j === i ? e.target.value : x)) })
                    }
                  />
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => patch(f, { options: f.options.filter((_, j) => j !== i) })}
                  >
              ...
              <Button
                ...
                variant="outline"
                onClick={() =>
                  patch(f, { options: [...f.options, `選択肢${f.options.length + 1}`] })
                }
              >
