...
export function OrgEduReview({ orgId }: { orgId: string }) {
  ...
  const load = async () => {
    ...
    const { data } = await (supabase as any).rpc("org_edu_review_rows", {
      _org: orgId,
      _include_done: showDone,
    });
    setRows(data ?? []);
  };
  useEffect(() => {
    load();
  }, [orgId, user?.id, showDone]);
      ...
      <Card className="p-4 flex items-center gap-2 flex-wrap">
        ...
        <div className="flex-1 min-w-[200px]">
          ...
          <div className="text-[11px] text-muted-foreground">
            学校の教材で間違えた問題をAIが解説します（コイン・XPは付きません）。
          </div>
        </div>
            ...
            <div className="pt-1">
              {text ? (
                <div className="rounded-lg bg-primary/5 border border-primary/20 p-3 text-sm whitespace-pre-wrap">
                  {text}
                  {busy === r.id && <span className="animate-pulse">▍</span>}
                </div>
              ) : (
                <Button
                  size="sm"
