...
export function OrgManage({
  ...
  const load = async () => {
    ...
    if (admin) {
      const [{ data: r }, { data: pi }, { data: jr }, { data: st }] = await Promise.all([
        (supabase as any)
          .from("organization_service_restrictions")
          .select("*")
          .eq("organization_id", orgId),
        (supabase as any)
  ...
  const reviewReq = async (id: string, approve: boolean, role = "member") => {
    const { error } = await (supabase as any).rpc("org_review_join_request", {
      _req_id: id,
      _approve: approve,
      _role: role,
    });
  ...
  const enrollAll = async (classId: string) => {
    const { data, error } = await (supabase as any).rpc("org_enroll_all", {
      _org: orgId,
      _class: classId,
    });
    if (error) return toast.error(error.message);
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {org?.status !== "approved" && (
          <span className="text-xs px-2 py-0.5 rounded bg-amber-500/20 text-amber-700">
            運営の承認待ち
          </span>
        )}
        <span className="text-xs px-2 py-0.5 rounded bg-muted">
          あなた: {ROLE_LABEL[myRole ?? ""] ?? "運営"}
        </span>
      </div>
      {canAdmin && (
        <Card className="p-3 flex items-center gap-2 text-sm">
          ...
          参加コード:{" "}
          <span className="font-mono text-lg font-bold tracking-widest">{org?.join_code}</span>
          <Button
            size="sm"
            variant="ghost"
            onClick={() => {
      ...
      <Tabs defaultValue={defaultTab}>
            ...
            <Card key={m.id} className="p-3 flex flex-wrap items-center gap-2">
              ...
              {canAdmin && m.role !== "owner" && (
                <>
                  <Button
                    size="sm"
                    variant={m.suspended ? "default" : "outline"}
        ...
        {canAdmin && (
          <TabsContent value="requests" className="space-y-2">
            {joinReqs.length === 0 && (
              <Card className="p-6 text-sm text-muted-foreground">参加申請はありません</Card>
            )}
            {joinReqs.map((r: any) => (
              <Card key={r.id} className="p-3 flex flex-wrap items-center gap-2">
                ...
                <Button size="sm" onClick={() => reviewReq(r.id, true, "member")}>
                  一般として承認
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => reviewReq(r.id, true, "teacher")}
                >
                  教師として承認
                </Button>
                <Button
        ...
        {canAdmin && (
          <TabsContent value="stats" className="space-y-2">
            <div className="text-xs text-muted-foreground">
              メンバーの学習サマリー（閲覧のみ・記録の編集はできません）
            </div>
            {stats.map((s: any) => (
              <Card key={s.user_id} className="p-3 flex flex-wrap items-center gap-3 text-sm">
                <div className="flex-1 min-w-[140px] font-medium">
                  {s.display_name ?? s.username ?? s.user_id.slice(0, 8)}
                  <span className="text-[10px] ml-2 px-1.5 rounded bg-muted">
                    {ROLE_LABEL[s.role] ?? s.role}
                  </span>
                </div>
                <div>
                  7日: <b>{fmt(Number(s.minutes_7d))}</b>
                </div>
                <div>
                  30日: <b>{fmt(Number(s.minutes_30d))}</b>
                </div>
                <div className="text-xs text-muted-foreground">
                  記録 {s.sessions_30d}件 / 最終{" "}
                  {s.last_studied ? new Date(s.last_studied).toLocaleDateString("ja-JP") : "—"}
                </div>
              </Card>
            ))}
            {stats.length === 0 && (
              <Card className="p-6 text-sm text-muted-foreground">データがありません</Card>
            )}
          </TabsContent>
        )}
        ...
        <TabsContent value="content" className="space-y-3">
          <Card className="p-4 space-y-2">
            ...
            {packs.map((p: any) => (
              ...
              >
                <span>
                  {p.title}{" "}
                  <span className="text-[10px] ml-1 px-1.5 rounded bg-muted">{p.status}</span>
                </span>
                <Link
                  to="/makron/pack/$packId"
                  params={{ packId: p.id }}
            ...
            {packs.length === 0 && (
              <div className="text-xs text-muted-foreground">
                まだありません。Makronで問題集を作成後、下のボタンで組織に紐づけできます。
              </div>
            )}
          ...
          <Card className="p-4 space-y-2">
            ...
            {classes.map((c: any) => (
              ...
              >
                ...
                <div className="flex gap-2">
                  {canAdmin && (
                    <Button size="sm" variant="outline" onClick={() => enrollAll(c.id)}>
                      全メンバーを強制参加
                    </Button>
                  )}
                  <Link
                    ...
                    params={{ classId: c.id }}
                    className="underline text-xs self-center"
                  >
                    開く →
                  </Link>
                </div>
            ...
            {classes.length === 0 && (
              <div className="text-xs text-muted-foreground">
                まだありません。クラスルームを作成後、下から組織に紐づけできます。
              </div>
            )}
          ...
          <TabsContent value="invite" className="space-y-2">
              ...
              <div className="space-y-1 max-h-60 overflow-auto">
                {inviteResults.map((u: any) => (
                  <div key={u.id} className="flex items-center gap-2 border rounded p-2 text-sm">
                    ...
                    <Button size="sm" onClick={() => invite(u.id)}>
            ...
            {pendingInvites.map((p: any) => (
              <Card key={p.id} className="p-3 flex items-center justify-between text-sm">
                <div>
                  <div className="font-medium">
                    {p.profile?.display_name ?? p.profile?.username ?? p.invitee_id.slice(0, 8)}
                  </div>
                  <div className="text-[10px] text-muted-foreground">
            ...
            {restrictions.map((r: any) => (
              <Card key={r.id} className="p-3 flex items-center justify-between">
                <div>
                  <div className="font-medium">
                    {r.service_key}{" "}
                    <span className="text-[10px] ml-1 px-1.5 rounded bg-muted">{r.variant}</span>
                  </div>
    ...
    <div className="flex gap-2 pt-2">
      <Select value={sel} onValueChange={setSel}>
        <SelectTrigger className="h-9 flex-1">
          <SelectValue placeholder="自分の問題集を組織に紐づけ" />
        </SelectTrigger>
        <SelectContent>
          {mine.map((p: any) => (
...
function LinkClassForm({ orgId, onDone }: { orgId: string; onDone: () => void }) {
  ...
  return (
    <div className="flex gap-2 pt-2">
      <Select value={sel} onValueChange={setSel}>
        <SelectTrigger className="h-9 flex-1">
          <SelectValue placeholder="自分のクラスを組織に紐づけ" />
        </SelectTrigger>
        <SelectContent>
          {mine.map((c: any) => (
