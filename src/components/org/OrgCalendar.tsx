...
export function OrgCalendar({ orgId, ctx }: { orgId: string; ctx: any }) {
  ...
  const create = async () => {
    ...
    await (supabase as any).rpc("org_notify_members", {
      ...
      _app: "calendar",
      _title: `予定: ${title.trim()}`,
      _body: new Date(start).toLocaleString("ja-JP"),
    });
  ...
  const visible = rows.filter(
    (r) => filter === "all" || (filter === "org" ? !r.group_id : r.group_id === filter),
  );
  ...
  const EventRow = ({ e }: { e: any }) => (
    <Card className="p-3 flex flex-wrap items-start gap-3">
      <span
        ...
        style={{ background: colorOf(e.group_id) }}
      />
      <div className="flex-1 min-w-[180px]">
        <div className="font-medium">
          {e.title}
          {e.group_id && (
            <span className="text-[10px] ml-2 px-1.5 rounded bg-sky-500/15 text-sky-600">
              {gname(e.group_id)}
            </span>
          )}
        </div>
        <div className="text-[11px] text-muted-foreground">
          {new Date(e.starts_at).toLocaleString("ja-JP")}
          {e.ends_at && ` 〜 ${new Date(e.ends_at).toLocaleString("ja-JP")}`}
        </div>
        {e.location && (
      ...
      {(e.created_by === user?.id || ctx.canAdmin) && (
        <Button
          ...
          onClick={async () => {
            await (supabase as any).from("org_calendar_events").delete().eq("id", e.id);
            load();
          }}
        >
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2 items-center">
        <OrgScopePicker
          groups={[{ id: "all", name: "すべて" }, ...ctx.groups]}
          value={filter}
          onChange={setFilter}
          orgLabel="組織カレンダー"
        />
        <div className="flex rounded-md border overflow-hidden">
          <button
            className={`px-2.5 py-1.5 text-xs flex items-center gap-1 ${mode === "month" ? "bg-primary text-primary-foreground" : ""}`}
            onClick={() => setMode("month")}
          >
            <LayoutGrid className="h-3.5 w-3.5" />月
          </button>
          <button
            className={`px-2.5 py-1.5 text-xs flex items-center gap-1 ${mode === "list" ? "bg-primary text-primary-foreground" : ""}`}
      ...
      {mode === "month" ? (
        <div className="grid lg:grid-cols-[1fr_300px] gap-4">
                    ...
                    {d && (
                      <>
                        <div
                          ...
                          ${key === todayKey ? "bg-primary text-primary-foreground font-bold" : i % 7 === 0 ? "text-rose-500" : i % 7 === 6 ? "text-sky-500" : ""}`}
                        >
                          {d.getDate()}
                        </div>
          ...
          <div className="space-y-2">
            ...
            {dayEvents.length === 0 && (
              <Card className="p-6 text-xs text-muted-foreground text-center">
                予定はありません
              </Card>
            )}
            {dayEvents.map((e: any) => (
        ...
        <div className="space-y-2">
          {visible.length === 0 && (
            <Card className="p-6 text-sm text-muted-foreground">予定はありません</Card>
          )}
          {visible.map((e) => (
            <EventRow key={e.id} e={e} />
          ))}
