...
export function OrgEdu({ orgId, ctx }: { orgId: string; ctx: any }) {
  ...
  const load = async () => {
    ...
    const [f, vals, { data: s }, { data: u }, { data: q }, { data: at }, { data: st }] =
      await Promise.all([
        loadOrgFields(orgId),
        ...
        (supabase as any)
          ...
          .select(
            "id, organization_id, unit_id, kind, body, choices, hint_text, audience, level, sort_order, created_by, created_at, updated_at",
          )
          .eq("organization_id", orgId)
          .order("sort_order"),
        (supabase as any)
    ...
    if (isStaff) {
      ...
      qs = qs.map((x) => ({
        ...x,
        answer: km[x.id]?.answer ?? "",
        explanation: km[x.id]?.explanation ?? "",
      }));
    }
    setQuestions(qs);
  ...
  useEffect(() => {
    if (user) load();
  }, [orgId, user?.id, ctx.org?.current_year]);

  const visibleUnits = useMemo(
    () =>
      units
        .filter((u) => u.subject_id === subjectId)
        .filter((u) => isStaff || matchAudience(u.audience, myValues)),
    [units, subjectId, myValues, isStaff],
  );

  const qOfUnit = (unitId: string) =>
    questions
  ...
  const progressPct = Math.min(
    100,
    Math.round(
      ((xp - nextLevelXp(lv - 1)) / Math.max(1, nextLevelXp(lv) - nextLevelXp(lv - 1))) * 100,
    ),
  );
      ...
      <Card className="p-5 bg-gradient-to-br from-primary/20 via-primary/5 to-transparent border-primary/30">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex-1 min-w-[180px]">
            ...
            <div className="text-xs text-muted-foreground">
              組織専用の教材です（コイン・ポイントは貯まりません）
            </div>
          </div>
          {isStaff && (
            <Button size="sm" variant="outline" onClick={() => setMode("manage")}>
        ...
        <div className="mt-4 grid grid-cols-3 gap-3 text-center">
          ...
          <Stat
            ...
            label="連続学習"
            value={`${streak?.current_streak ?? 0}日`}
          />
          <Stat
            icon={<Trophy className="h-4 w-4 text-primary" />}
            label="クリア単元"
            value={`${clearedUnits.size}/${units.length}`}
          />
        </div>
        <div className="mt-3">
          <Progress value={progressPct} className="h-2" />
          <div className="text-[10px] text-muted-foreground mt-1">
            次のレベルまで {Math.max(0, nextLevelXp(lv) - xp)} XP ・ 復習待ち {mistakes} 問
          </div>
        </div>
      </Card>
      ...
      <div className="flex flex-wrap gap-2">
        {subjects.map((s, i) => (
          <button
            ...
            style={{
              borderColor: s.color ?? HUES[i % HUES.length],
              background: subjectId === s.id ? (s.color ?? HUES[i % HUES.length]) : "transparent",
            }}
          >
            {s.name}
          </button>
        ...
        {subjects.length === 0 && (
          <Card className="p-6 w-full text-center text-sm text-muted-foreground">
            まだ教科がありません{isStaff ? "。「問題を管理」から追加できます。" : ""}
          </Card>
        )}
      </div>
      ...
      <div className="space-y-1">
        ...
        <div className="relative pt-3">
          {visibleUnits.map((u, i) => {
            ...
            const doneCount = qs.filter((q) =>
              attempts.some((a) => a.question_id === q.id && a.correct),
            ).length;
            return (
              <div key={u.id} className="flex flex-col items-center">
                {i > 0 && (
                  <div className="h-6 w-1 rounded-full bg-border" style={{ marginLeft: off }} />
                )}
                <div
                ...
                >
                  ...
                  >
                    {cleared ? (
                      ...
                      <Star className="h-7 w-7" style={{ color: hue }} />
                    ) : (
                      <Lock className="h-6 w-6 text-muted-foreground" />
                    )}
                    <span className="absolute -bottom-1 text-[9px] px-1.5 rounded-full bg-background border">
                      Lv{u.level}
                    </span>
                  </button>
                  <div className="max-w-[200px]">
                    ...
                    <div className="text-[11px] text-muted-foreground">
                      {doneCount}/{qs.length} 問正解
                      {isStaff ? ` ・ ${audienceLabel(u.audience, fields)}` : ""}
                    </div>
                  </div>
                </div>
              </div>
    ...
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <Card className="p-5 space-y-4">
          ...
          <div className="grid gap-2">
            {(q.choices ?? []).map((c: string) => (
              <button
                ...
                onClick={() => setAnswer(c)}
                className={`text-left rounded-xl border-2 px-3 py-2 text-sm transition ${answer === c ? "border-primary bg-primary/10" : "hover:bg-muted"}`}
              >
                {c}
              </button>
            ))}
        ...
        {q.hint_text &&
          checked === null &&
          (showHint ? (
            <div className="text-xs rounded-lg bg-muted p-2">ヒント: {q.hint_text}</div>
          ) : (
        ...
        {checked !== null && (
          <div
            className={`rounded-xl p-3 text-sm ${checked ? "bg-emerald-500/10 text-emerald-600" : "bg-destructive/10 text-destructive"}`}
          >
            <div className="font-bold">
              {checked ? "正解！" : `不正解（正解: ${reveal?.answer ?? ""}）`}
            </div>
            {reveal?.explanation && (
            ...
            {!checked && (
              <div className="mt-1 text-[11px] text-muted-foreground">
                この問題は「間違い直しノート」に入ります。AIが解き方を教えてくれます。
              </div>
            )}
          </div>
        )}
...
function Manage({
  ...
  useEffect(() => {
    (async () => {
      ...
      for (const r of data ?? [])
        map[r.id] = { answer: r.answer ?? "", explanation: r.explanation ?? "" };
      setKeys(map);
    })();
  }, [orgId, questions.length]);

  const selectFields = fields.filter((f) => f.type === "select");
  const toggleAud = (setter: any, key: string, opt: string) =>
    setter((s: any) => {
  ...
  const AudPicker = ({
    ...
    onToggle: (k: string, o: string) => void;
  }) => (
    <div className="space-y-1">
        ...
        <div key={f.id} className="flex flex-wrap items-center gap-1">
          ...
          {f.options.map((o) => (
            <button
              ...
              className={`px-2 py-0.5 rounded-full text-[11px] border ${(value[f.key] ?? []).includes(o) ? "bg-primary text-primary-foreground border-primary" : "hover:bg-muted"}`}
            >
              {o}
            </button>
          ))}
      ...
      {selectFields.length === 0 && (
        <div className="text-[11px] text-muted-foreground">
          「プロフィール項目」で選択式の項目（例: 学年）を作ると、対象を絞れます。
        </div>
      )}
    </div>
  );
  ...
  const addSubject = async () => {
    ...
    const { error } = await (supabase as any).from("org_edu_subjects").insert({
      ...
      color: HUES[subjects.length % HUES.length],
  ...
  const saveQuestion = async () => {
    ...
    const payload = {
      ...
      hint_text: draft.hint_text || null,
          ...
          <Card className="p-4 space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {subjects.map((s) => (
                ...
                >
                  {s.name}
                  <button onClick={() => del("org_edu_subjects", s.id)}>
              ...
              {unitsOfSubject.map((u) => (
                ...
                >
                  ...
                  <span className="ml-auto">
                    {questions.filter((q) => q.unit_id === u.id).length}問
                  </span>
                  <button onClick={() => del("org_edu_units", u.id)}>
                    <Trash2 className="h-3 w-3 text-destructive" />
                  </button>
                </div>
              ))}
        ...
        <Card className="p-4 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            <Select
              ...
              onValueChange={(v) => {
                setSubjectId(v);
                const first = units.find((u) => u.subject_id === v);
                setUnitId(first?.id ?? "");
              }}
            >
              <SelectTrigger className="w-40 h-9">
            ...
            <Button size="sm" onClick={newDraft} disabled={!unitId}>
              <Plus className="h-4 w-4 mr-1" />
              問題を追加
            </Button>
          </div>
            ...
            {questions
              ...
              .map((q, i) => (
                ...
                >
                  ...
                  <button onClick={() => del("org_edu_questions", q.id)}>
                    <Trash2 className="h-3 w-3 text-destructive" />
                  </button>
                </div>
              ))}
            ...
            <Card className="p-4 space-y-3 bg-muted/30">
              ...
              <div className="grid sm:grid-cols-2 gap-2">
                <div>
                  ...
                  <Select
                    value={draft.unit_id}
                    onValueChange={(v) => setDraft({ ...draft, unit_id: v })}
                  >
                ...
                <div>
                  ...
                  <Select
                    value={draft.kind}
                    onValueChange={(v) => setDraft({ ...draft, kind: v, answer: "" })}
                  >
                    <SelectTrigger className="h-9">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      ...
                      <SelectItem value="text">記述</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
                ...
                <div className="space-y-1">
                  ...
                  {draft.choices.map((o: string, i: number) => (
                    <div key={i} className="flex items-center gap-2">
                      <input
                        type="radio"
                        checked={!!o && draft.answer === o}
                        onChange={() => o && setDraft({ ...draft, answer: o })}
                      />
                      <Input
                        value={o}
                        placeholder={`選択肢${i + 1}`}
                        onChange={(e) => {
                          const cs = [...draft.choices];
                          const prev = cs[i];
                          cs[i] = e.target.value;
                          setDraft({
                            ...draft,
                            choices: cs,
                            answer: draft.answer === prev ? e.target.value : draft.answer,
                          });
                        }}
                      />
                      <Button
                        ...
                        onClick={() =>
                          setDraft({
                            ...draft,
                            choices: draft.choices.filter((_: any, j: number) => j !== i),
                ...
                onToggle={(k, o) =>
                  setDraft((d: any) => {
                    ...
                    return {
                      ...
                      audience: {
                        ...(d.audience ?? {}),
                        [k]: list.includes(o) ? list.filter((x) => x !== o) : [...list, o],
                      },
                    };
                  })
                }
