...
export function MaterialPicker({
  ...
  const favOnly = useMemo(
    () => sortByFav(filtered.filter((m) => favIds.has(m.id))),
    [filtered, favIds],
  );
      ...
      <div className="space-y-2">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
            ...
            <Input
              placeholder="タイトル/出版社で検索"
              value={q}
              onChange={(e) => setQ(e.target.value)}
            />
            <Tabs
              defaultValue={favOnly.length > 0 ? "fav" : "all"}
              className="flex-1 flex flex-col min-h-0"
            >
              <TabsList>
                <TabsTrigger value="fav">
                  <Star className="h-3 w-3 mr-1" />
                  お気に入り ({favOnly.length})
                </TabsTrigger>
                <TabsTrigger value="all">すべて ({allSorted.length})</TabsTrigger>
                <TabsTrigger value="selected">選択中 ({value.length})</TabsTrigger>
              </TabsList>
              <TabsContent value="fav" className="flex-1 overflow-auto">
                <Grid items={favOnly} value={value} toggle={toggle} favIds={favIds} />
              </TabsContent>
              <TabsContent value="all" className="flex-1 overflow-auto">
                <Grid items={allSorted} value={value} toggle={toggle} favIds={favIds} />
              </TabsContent>
              <TabsContent value="selected" className="flex-1 overflow-auto">
                <Grid items={selected} value={value} toggle={toggle} favIds={favIds} />
              </TabsContent>
            </Tabs>
            ...
            {filtered.map((m) => (
              ...
              >
                ...
                <div className="text-sm">
                  <div>{m.title}</div>
                  {m.publisher && (
                    <div className="text-xs text-muted-foreground">{m.publisher}</div>
                  )}
                </div>
              </label>
            ))}
        ...
        return (
          ...
          >
            {m.cover_url ? (
              ...
              <div className="w-12 h-16 rounded bg-muted text-[10px] text-muted-foreground flex items-center justify-center shrink-0">
                No img
              </div>
            )}
            <div className="min-w-0 flex-1">
              ...
              {m.subject && (
                <div className="text-[10px] text-muted-foreground mt-0.5">{m.subject}</div>
              )}
              {m.publisher && (
                <div className="text-[10px] text-muted-foreground truncate">{m.publisher}</div>
              )}
            </div>
          </button>
        );
