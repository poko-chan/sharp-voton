...
export function RestrictionProvider({ children }: { children: ReactNode }) {
  ...
  const load = async () => {
    const [{ data: g }, { data: u }] = await Promise.all([
      supabase
        ...
        .select("service_key, restricted, message, restricted_until"),
      user
        ? supabase
    ...
    const expired = (r: any) =>
      r.restricted_until && new Date(r.restricted_until).getTime() < Date.now();
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "service_restrictions" }, () =>
        load(),
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "user_service_restrictions" },
        () => load(),
      )
      .subscribe();
...
export function useServiceStatus(serviceKey: string) {
  ...
  if (g)
    return {
      ...
      message: g.message,
      until: g.restricted_until,
    };
  if (u)
    return {
      blocked: true as const,
      variant: "restrict" as const,
      message: u.message,
      until: u.restricted_until,
    };
