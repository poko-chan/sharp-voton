  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await supabase
      ...
      .insert({
        ...
        author_id: userId,
        body: data.body,
        private_to: data.privateTo ?? null,
      } as any)
      .select()
      .single();
    if (error) throw new Error(error.message);
...
export const submitQuiz = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: visible, error: e1 } = await supabase
      .from("assignments")
      .select("id, kind")
    ...
    const keyMap = new Map<string, string>(
      (((asg as any).quiz_answer_key as any[]) ?? []).map((k: any) => [
        String(k.id),
        String(k.answer ?? ""),
      ]),
    );
    const questions = (asg.quiz_questions as any[]).map((q: any) => ({
      ...q,
      answer: keyMap.get(String(q.id)) ?? q.answer,
    }));
    let totalPts = 0;
    let earnedPts = 0;
    const detail: any[] = [];
    ...
    for (const q of questions) {
      ...
      const correct =
        String(q.answer ?? "")
          .trim()
          .toLowerCase() === userAns.trim().toLowerCase();
      if (correct) earnedPts += pts;
      detail.push({ questionId: q.id, userAnswer: userAns, correct, expected: q.answer });
    }
    const score = totalPts > 0 ? Math.round((earnedPts / totalPts) * (asg.max_points ?? 100)) : 0;
    let xp = 0;
