...
export const getStudyContext = createServerFn({ method: "POST" })
  .inputValidator((i) =>
    z
      .object({ scopes: z.array(z.enum(STUDY_SCOPES)).optional() })
      .optional()
      .parse(i ?? {}),
  )
  ...
  .handler(async ({ data, context }) => {
    ...
    const [logsR, subjR, goalsR, qR, profR, examsR, todosR, cardsR, packsR] = await Promise.all([
      ...
      on("goals")
        ? supabase
            .from("goals")
            .select("title, target_minutes, progress_minutes, deadline, done")
            .eq("user_id", userId)
            .eq("done", false)
            .limit(10)
        : empty,
      on("weak")
        ? supabase
            .from("questions")
            .select("topic, was_wrong, attempts")
            .eq("user_id", userId)
            .limit(500)
        : empty,
      ...
      on("flashcards")
        ? (supabase as any)
            .from("flashcards")
            .select("front, reviews, ease, next_review_at")
            .eq("user_id", userId)
      ...
      on("markon")
        ? (supabase as any)
            .from("makron_pack_attempts")
            .select("attempts_count, xp_earned_total, last_attempt_at, makron_packs(title)")
            .eq("user_id", userId)
    ...
    const topSubjects = Array.from(subjAgg.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
    ...
    const recentNotes = on("notes")
      ? logs
          ...
          .map(
            (l) =>
              `- ${l.date}: ${l.subjects?.name ?? ""} ${l.duration_minutes}分 ${l.content ?? ""}`,
          )
          .join("\n")
      : "";
    ...
    return {
      ...
      recentNotes,
      upcomingExams: (examsR.data ?? []).map((e: any) => ({
        title: e.name,
        date: e.start_date,
        end: e.end_date,
      })),
      examTodos: (todosR.data ?? []).map((t: any) => t.text),
      hardCards: (cardsR.data ?? [])
        .filter((c: any) => (c.reviews ?? 0) > 0)
        .map((c: any) => ({
          front: c.front,
          ease: c.ease,
          reviews: c.reviews,
      ...
      markonRecent: (packsR.data ?? []).map((a: any) => ({
        pack: a.makron_packs?.title ?? "パック",
        attempts: a.attempts_count,
        xp: a.xp_earned_total,
        at: a.last_attempt_at,
      })),
    };
  });
...
export const createTutorThread = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: row, error } = await (context.supabase as any)
      .from("tutor_threads")
      .insert({ user_id: context.userId, title: data.title ?? "新しいチャット" })
      .select()
      .single();
    if (error) throw error;
    return row;
