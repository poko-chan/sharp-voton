...
export const createClass = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await supabase
      .from("class_members")
      .insert({ class_id: cls.id, user_id: userId, role: "teacher" });
    return cls;
  });
...
export const joinClass = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: r, error } = await supabase.rpc("join_class_by_code" as any, {
...
export const gradeSubmission = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: asg, error: e2 } = await supabase
      .from("assignments")
      .select("xp_mode, fixed_xp, max_points, class_id")
