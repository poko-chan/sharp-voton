...
export const Route = createFileRoute("/_authenticated/organizations/$orgId/profile")({
...
function OrgProfilePage() {
  ...
  useEffect(() => {
    if (org?.current_year) setYear(org.current_year);
  }, [org?.current_year]);
  ...
  const save = async () => {
    ...
    if (editable.length) {
      const merged = {
        ...(await loadOrgYearValues(orgId, year, [user!.id]))[user!.id],
        ...Object.fromEntries(editable.map((f) => [f.key, values[f.key] ?? ""])),
      };
  ...
  return (
    <div className="max-w-xl mx-auto p-6 space-y-3">
      ...
      <p className="text-xs text-muted-foreground">
        この情報は組織の中でのみ表示されます（通常のStudy#プロフィールとは別です）。
      </p>
      <Card className="p-4 space-y-3">
        <div>
          ...
          <Input
            ...
            onChange={(e) => setP((s: any) => ({ ...s, display_name: e.target.value }))}
          />
        </div>
        ...
        <Textarea
          ...
          placeholder="自己紹介（任意）"
          value={p.bio ?? ""}
          onChange={(e) => setP((s: any) => ({ ...s, bio: e.target.value }))}
        />
