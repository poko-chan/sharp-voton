...
export const Route = createFileRoute("/_authenticated/makron/pack/$packId/dashboard")({
...
function PackDashboard() {
  ...
  const load = async () => {
    ...
    setStats(s);
    setAttempters(a ?? []);
    const { data: ses, error: sesErr } = await (supabase as any)
  ...
  const resetAll = async () => {
    ...
    const { error } = await (supabase as any).rpc("makron_pack_reset_attempts", {
      _pack_id: packId,
      _user_id: null,
    });
  ...
  const saveSession = async (sid: string, score: number, passed: boolean) => {
    const { error } = await (supabase as any).rpc("makron_update_session", {
      _session_id: sid,
      _score: score,
      _passed: passed,
    });
  ...
  const visibleSessions =
    filterUser === "__all" ? sessions : sessions.filter((s) => s.user_id === filterUser);

  return (
    <MakronShell back="/makron/units" title="パックダッシュボード" subtitle={pack?.title}>
      <div className="max-w-6xl mx-auto p-6 space-y-4">
        <div className="flex items-center gap-2">
          <Link to="/makron/pack/$packId" params={{ packId }}>
                ...
                <div key={a.user_id} className="p-3 flex items-center gap-2 text-sm">
                  <div className="flex-1">
                    ...
                    <div className="text-[11px] text-muted-foreground">
                      演習{a.attempts_count}回 ・ 最高{a.best_score}点 ・ 最終
                      {a.last_attempt_at
                        ? new Date(a.last_attempt_at).toLocaleString("ja-JP")
                        : "-"}
                    </div>
                  </div>
                  <Button
                    ...
                    variant="outline"
                    onClick={() => {
                      setFilterUser(a.user_id);
                    }}
                  >
                    セッション表示
                  </Button>
                  <Button size="sm" variant="ghost" onClick={() => resetUser(a.user_id)}>
            ...
            <div className="flex items-center gap-2">
              <Select value={filterUser} onValueChange={setFilterUser}>
                <SelectTrigger className="w-64">
                  <SelectValue placeholder="演習者で絞り込み" />
                </SelectTrigger>
                <SelectContent>
              ...
              {visibleSessions.map((s: any) => (
                <div key={s.id} className="p-3 flex items-center gap-2 text-sm">
                  ...
                  <Input
                    type="number"
                    className="w-20"
                    value={editScore[s.id] ?? s.total_score ?? 0}
                    onChange={(e) =>
                      setEditScore((p) => ({ ...p, [s.id]: Number(e.target.value) }))
                    }
                  />
                  <span className="text-[11px] text-muted-foreground">/ {s.total_points ?? 0}</span>
                  <Select
                    value={s.passed === true ? "1" : s.passed === false ? "0" : "_"}
                    onValueChange={(v) =>
                      saveSession(s.id, editScore[s.id] ?? s.total_score ?? 0, v === "1")
                    }
                  >
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      saveSession(s.id, editScore[s.id] ?? s.total_score ?? 0, s.passed === true)
                    }
                  >
