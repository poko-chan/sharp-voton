...
function DueBadge({ dueAt }: { dueAt: string | null }) {
  ...
  if (d < 0)
    return (
      <span className="text-[11px] px-2 py-0.5 rounded-full bg-destructive/15 text-destructive border border-destructive/30">
        期限切れ
      </span>
    );
  if (d === 0)
...
function ClassDetail() {
  ...
  const load = async () => {
    ...
    const { data: profs } =
      userIds.length > 0 ? { data: await fetchPublicProfiles(userIds) } : { data: [] as any[] };
    ...
    const { data: asg } = await supabase
      .from("assignments")
      .select(
        "id, class_id, created_by, title, description, due_at, max_points, xp_mode, fixed_xp, attachments, allowed_file_types, kind, quiz_questions, created_at, updated_at",
      )
      .eq("class_id", classId)
  ...
  const upcoming = useMemo(() => {
    ...
    return assignments
      .filter(
        (a) =>
          a.due_at &&
          new Date(a.due_at).getTime() - now < 7 * 24 * 60 * 60 * 1000 &&
          new Date(a.due_at).getTime() > now - 24 * 60 * 60 * 1000,
      )
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-5">
      ...
      <Card className="p-5 space-y-3">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={copyCode}
              className="flex items-center gap-2 px-3 py-2 rounded-lg border bg-muted/30 text-sm font-mono hover:bg-muted"
            >
        ...
        {isTeacher && (
          <div className="flex flex-wrap gap-2 pt-2 border-t">
            <Button size="sm" variant="secondary" onClick={() => setTab("stream")}>
              <Megaphone className="h-3 w-3 mr-1" />
              お知らせ
            </Button>
            <Button size="sm" variant="secondary" onClick={() => setTab("assignments")}>
            ...
            <Button size="sm" variant="secondary" onClick={() => setTab("permissions")}>
      ...
      <Card className="p-4">
        ...
        {upcoming.length === 0 && (
          <p className="text-xs text-muted-foreground">今週の締切はありません</p>
        )}
          ...
          {upcoming.map((a) => {
            ...
            return (
              <button
                ...
                className="text-left p-2.5 rounded-lg border bg-muted/20 hover:bg-muted/40 transition space-y-1"
              >
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <div className="-mx-4 px-4 md:mx-0 md:px-0 overflow-x-auto">
          <TabsList className="flex-nowrap h-auto w-max min-w-full md:w-auto">
            <TabsTrigger value="stream" className="whitespace-nowrap">
              <Megaphone className="h-3 w-3 mr-1" />
              ストリーム
            </TabsTrigger>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted">
                <tr className="text-left">
                  ...
                  <th className="p-3">参加日</th>
                  {isTeacher && <th className="p-3">操作</th>}
                </tr>
              </thead>
              <tbody>
                {members.map((m) => (
