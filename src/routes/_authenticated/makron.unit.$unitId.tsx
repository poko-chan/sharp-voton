...
export const Route = createFileRoute("/_authenticated/makron/unit/$unitId")({
  component: UnitPage,
});
...
function UnitPage() {
  ...
  return (
    <MakronShell
      back="/makron/units"
      title={unit?.title ?? "単元"}
      subtitle={[unit?.subject, unit?.field, unit?.unit].filter(Boolean).join(" / ")}
    >
      <div className="max-w-4xl mx-auto p-6 space-y-4">
        {unit?.description && (
          <Card className="p-4 text-sm whitespace-pre-wrap">{unit.description}</Card>
        )}
        {isAdmin && (
            ...
            return (
              <Card key={p.id} className="p-4 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  {p.grade && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted">{p.grade}</span>
                  )}
                </div>
                {p.description && (
                  ...
                  {isOwner && (
                    <>
                      <Link to="/makron/pack/$packId" params={{ packId: p.id }}>
                        <Button size="sm" variant="outline" title="編集・設定">
                          <Settings className="h-4 w-4" />
                        </Button>
                      </Link>
                      <Link to="/makron/pack/$packId/dashboard" params={{ packId: p.id }}>
                        <Button size="sm" variant="outline" title="ダッシュボード">
