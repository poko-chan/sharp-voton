...
function ClassChatPage() {
  ...
  const loadMessages = async (cid: string) => {
    ...
    if (ids.length) {
      const { data: pf } = await supabase.rpc("public_profiles_by_ids", { _ids: ids });
              ...
              {msgs.map((m) => {
                ...
                return (
                  <div key={m.id} className={`flex ${own ? "justify-end" : "justify-start"}`}>
                    <div
                      className={`max-w-[75%] rounded-2xl px-3 py-2 ${own ? "bg-primary text-primary-foreground" : "bg-muted"}`}
                    >
