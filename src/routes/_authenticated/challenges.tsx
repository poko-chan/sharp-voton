...
function Ch() {
  ...
  useEffect(()=>{load();}, []);
  const create = async () => { if(!user||!name)return; const { data } = await supabase.from("group_challenges").insert({ owner_id: user.id, name, week_start: new Date().toISOString().slice(0,10) }).select().single(); if(data) await supabase.from("group_challenge_members").insert({ challenge_id: data.id, user_id: user.id }); setName(""); load(); };
  ...
  return (<div className="p-4 md:p-8 max-w-4xl mx-auto space-y-4">
    ...
    {list.map(c => (<Card key={c.id} className="p-4">
      ...
      <div className="text-xs text-muted-foreground">週開始: {c.week_start} / メンバー {c.group_challenge_members?.length ?? 0}</div>
    </Card>))}
  </div>);
}