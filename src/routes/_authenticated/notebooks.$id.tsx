...
function NotebookEditor() {
  ...
  const invite = async () => {
    ...
    const target =
      found.find(
        (p) => (p.username ?? "").toLowerCase() === username.trim().replace(/^@/, "").toLowerCase(),
      ) ?? found[0];
      ...
      title={
        <span className="flex items-center gap-2">
          ...
          <Badge variant="secondary" className="hidden sm:inline-flex">
            {PAPER_TYPES.find((p) => p.key === nb.paper_type)?.label}
          </Badge>
          ...
          <Card className="absolute right-2 top-2 z-20 max-h-[80%] w-[min(420px,92vw)] space-y-4 overflow-y-auto p-4 shadow-2xl">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {PAPER_TYPES.map((p) => (
                  <button
                    key={p.key}
                    onClick={() => patchNotebook({ paper_type: p.key as PaperType })}
                    className={cn(
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {PAPER_COLORS.map((p) => (
                  <button
                    key={p.key}
                    onClick={() => patchNotebook({ paper_color: p.key })}
                    className={cn(
                      "rounded-md border-2 px-3 py-1.5 text-xs",
                      nb.paper_color === p.key ? "border-foreground" : "border-border",
                    )}
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                />
                <Button
                  variant={canEditShare ? "default" : "outline"}
                  size="sm"
                  onClick={() => setCanEditShare((v) => !v)}
                >
              ...
              <div className="space-y-1">
                {shares.map((s) => (
                  <div key={s.id} className="flex items-center gap-2 rounded border p-2 text-sm">
                    ...
                    <Button size="icon" variant="ghost" onClick={() => removeShare(s.id)}>
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  </div>
                ))}
                {shares.length === 0 && (
                  <p className="text-xs text-muted-foreground">まだ共有していません。</p>
                )}
              </div>
            </div>
            <Button
              variant="outline"
              size="sm"
              onClick={() => patchNotebook({ archived: !nb.archived })}
            >
              <Archive className="mr-1 h-4 w-4" />
              {nb.archived ? "アーカイブを解除" : "アーカイブする"}
            </Button>
          </Card>
        )}
