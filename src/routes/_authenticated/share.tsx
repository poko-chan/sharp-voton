...
function SharePage() {
  ...
  const create = async () => {
    ...
    const { error } = await supabase
      .from("share_tokens")
      .insert({ token, user_id: user.id, label: label || "保護者ビュー" });
  ...
  const exportCSV = async () => {
    ...
    const { data } = await supabase
      .from("study_logs")
      .select("date,start_time,duration_minutes,content,subjects(name)")
      .eq("user_id", user.id)
      .order("date");
          ...
          {tokens.map((t) => {
            ...
            return (
              <div key={t.token} className="flex items-center gap-2 p-2 border rounded">
                ...
                <Button size="icon" variant="ghost" onClick={() => remove(t.token)}>
