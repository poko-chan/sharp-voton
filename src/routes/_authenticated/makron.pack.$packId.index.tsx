...
export const Route = createFileRoute("/_authenticated/makron/pack/$packId/")({
  component: PackPage,
});
...
function PackPage() {
  ...
  const load = async () => {
    ...
    if (p?.unit_id) {
      const { data: u } = await (supabase as any)
        .from("makron_units")
        .select("id,title,subject,field,unit")
        .eq("id", p.unit_id)
        .maybeSingle();
      setUnitMeta(u);
    }
  ...
  const startSession = async () => {
    const { data, error } = await (supabase as any).rpc("makron_start_pack_session", {
      _pack_id: packId,
    });
  ...
  return (
    <MakronShell
      back="/makron/units"
      title={pack.title}
      subtitle={[pack.grade, unitMeta?.title].filter(Boolean).join(" / ") || undefined}
    >
          ...
          {isOwner && (
            <Button
              ...
              onClick={async () => {
                if (
                  !confirm(
                    `「${pack.title}」を完全に削除します。問題・履歴・回答もすべて消えます。よろしいですか？`,
                  )
                )
                  return;
                const { error } = await (supabase as any).rpc("delete_makron_pack", {
                  _pack_id: packId,
                });
                if (error) return toast.error(error.message);
              ...
              {draft && (
                <Card className="p-4 space-y-3 bg-muted/30">
                  ...
                  <div>
                    ...
                    />
                    {draft.image_url && (
                      <img src={draft.image_url} alt="" className="max-h-32 mt-1 border rounded" />
                    )}
                  </div>
                  ...
                  {(draft.type === "text" || draft.type === "ocr") && (
                    <div>
                      <label className="text-xs">
                        ...
                        {draft.type === "ocr" && " ※OCRで読み取った文字と照合します"}
                      </label>
                      <Textarea
                        rows={3}
              ...
              <Card className="p-4 space-y-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onBlur={() => saveSettings({ title: pack.title })}
                  />
                </div>
                <div>
                  ...
                  <Textarea
                    ...
                    onBlur={() => saveSettings({ description: pack.description })}
                  />
                </div>
                <label className="flex items-center gap-2 text-sm">
                  <Switch
                    checked={!!pack.shuffle}
                    onCheckedChange={(v) => saveSettings({ shuffle: v })}
                  />
                  問題をシャッフルして出題
                </label>
                <label className="flex items-center gap-2 text-sm">
                  <Switch
                    checked={!!pack.skip_preview}
                    onCheckedChange={(v) => saveSettings({ skip_preview: v })}
                  />
                  演習開始前のプレビューを表示しない
                </label>
                <label className="flex items-center gap-2 text-sm">
                  <Switch
                    checked={!!pack.per_question_grading}
                    onCheckedChange={(v) => saveSettings({ per_question_grading: v })}
                  />
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input
                      type="number"
                      min={0}
                      value={pack.question_limit ?? ""}
                      onChange={(e) =>
                        setPack({
                          ...pack,
                          question_limit: e.target.value ? Number(e.target.value) : null,
                        })
                      }
                      onBlur={() => saveSettings({ question_limit: pack.question_limit })}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onBlur={() => saveSettings({ max_attempts: pack.max_attempts })}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onBlur={() => saveSettings({ pass_score: pack.pass_score })}
                    />
                  </div>
                </div>
                <label className="flex items-center gap-2 text-sm">
                  <Switch
                    checked={pack.allow_all_mode !== false}
                    onCheckedChange={(v) => saveSettings({ allow_all_mode: v })}
                  />
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <Input
                        type="number"
                        min={0}
                        value={pack.xp_per_question ?? 10}
                        onChange={(e) =>
                          setPack({ ...pack, xp_per_question: Number(e.target.value) })
                        }
                        onBlur={() => saveSettings({ xp_per_question: pack.xp_per_question })}
                      />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <Input
                        type="number"
                        value={pack.xp_cap_per_user ?? ""}
                        onChange={(e) =>
                          setPack({
                            ...pack,
                            xp_cap_per_user: e.target.value ? Number(e.target.value) : null,
                          })
                        }
                    ...
                    <div>
                      ...
                      <Input
                        type="number"
                        value={pack.reward_attempts_cap ?? ""}
                        onChange={(e) =>
                          setPack({
                            ...pack,
                            reward_attempts_cap: e.target.value ? Number(e.target.value) : null,
                          })
                        }
                        onBlur={() =>
                          saveSettings({ reward_attempts_cap: pack.reward_attempts_cap })
                        }
                      />
                    </div>
                  </div>
