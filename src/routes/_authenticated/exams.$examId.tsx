    ...
    <div className="container mx-auto p-6 max-w-5xl space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <span className="text-sm text-muted-foreground">
          {exam.start_date ?? ""}
          {exam.end_date && exam.end_date !== exam.start_date ? ` 〜 ${exam.end_date}` : ""}
        </span>
        <Button size="sm" className="ml-auto" onClick={addSubject}>
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <Input
            type="date"
            value={s.exam_date ?? ""}
            onChange={(e) => save({ exam_date: e.target.value || null })}
          />
        </div>
        <div>
          <Label>順番</Label>
          <Input
            type="number"
            value={s.order_no}
            onChange={(e) => save({ order_no: Number(e.target.value) })}
          />
        </div>
        <div>
          <Label>試験時間（分）</Label>
          <Input
            ...
            onChange={(e) => save({ duration_min: e.target.value ? Number(e.target.value) : null })}
          />
        </div>
        <div>
          <Label>満点</Label>
          <Input
            type="number"
            value={s.max_score}
            onChange={(e) => save({ max_score: Number(e.target.value) })}
          />
        </div>
        <div>
          <Label>目標点</Label>
          <Input
            type="number"
            value={s.target_score ?? ""}
            onChange={(e) => save({ target_score: e.target.value ? Number(e.target.value) : null })}
          />
        </div>
      </div>
      ...
      <div className="border-t pt-3 space-y-3">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          <div>
            ...
            <Input
              type="number"
              value={s.actual_score ?? ""}
              onChange={(e) =>
                save({ actual_score: e.target.value ? Number(e.target.value) : null })
              }
            />
          </div>
          <div>
            <Label>勉強時間 満足度 (1-10)</Label>
            <Input
              type="number"
              min={1}
              max={10}
              value={s.time_satisfaction ?? ""}
              onChange={(e) =>
                save({ time_satisfaction: e.target.value ? Number(e.target.value) : null })
              }
            />
          </div>
          <div>
            <Label>勉強内容 満足度 (1-10)</Label>
            <Input
              type="number"
              min={1}
              max={10}
              value={s.content_satisfaction ?? ""}
              onChange={(e) =>
                save({ content_satisfaction: e.target.value ? Number(e.target.value) : null })
              }
            />
          </div>
        </div>
        <div>
          ...
          <Textarea
            rows={4}
            value={s.reflection ?? ""}
            onChange={(e) => setS({ ...s, reflection: e.target.value })}
            onBlur={(e) => save({ reflection: e.target.value || null })}
          />
        </div>
      </div>
...
function TodoList({ subjectId }: { subjectId: string }) {
  ...
  const del = async (id: string) => {
    await (supabase as any).from("exam_todos").delete().eq("id", id);
    load();
  };
