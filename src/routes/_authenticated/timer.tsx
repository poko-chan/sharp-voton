  ...
  head: () => ({
    meta: [
      { title: "学習タイマー | Study#" },
      {
        name: "description",
        content:
          "ストップウォッチ・カウントダウン・ポモドーロ・呼吸法。集中モードとリング表示で、勉強時間をそのまま学習記録に残せます。",
      },
      { property: "og:title", content: "学習タイマー | Study#" },
      {
        property: "og:description",
        content:
          "ストップウォッチ・ポモドーロ・呼吸法を1画面で。計測した時間はそのまま学習記録に保存されます。",
      },
...
function TodaySummary() {
  ...
  const goal =
    Number(
      typeof window !== "undefined" ? localStorage.getItem("dashboard.dailyGoalMinutes") : 0,
    ) || 120;
    ...
    <Card className="p-3.5 mb-3">
      <div className="flex items-center justify-between text-xs mb-1.5">
        <span className="text-muted-foreground">
          今日の記録 <b className="text-foreground">{min}分</b> / 目標 {goal}分
        </span>
...
function TimerPage() {
  ...
  const initialTab =
    state?.kind === "countdown" ? "timer" : state?.kind === "pomodoro" ? "pomodoro" : "stopwatch";
  ...
  return (
    <div
      className={cn(
        "p-4 md:p-6 max-w-xl mx-auto",
        focus && "fixed inset-0 z-50 max-w-none bg-background overflow-auto p-6",
      )}
    >
    ...
    <Card className="p-5 mt-3 space-y-4 text-center">
      <Ring progress={isThis ? (elapsedMs % 3600000) / 3600000 : 0} running={!!running}>
        <div className="text-4xl font-mono font-bold tabular-nums tracking-tight">
          {fmtMs(isThis ? elapsedMs : 0)}
        </div>
        <div className="text-xs text-muted-foreground mt-1">
          {isThis ? `${min}分 経過` : "経過時間なし"}
        </div>
      </Ring>
      ...
      <div className="flex justify-center gap-3 flex-wrap">
        {!isThis ? (
          ...
          <Button size="lg" onClick={resume}>
            <Play className="mr-2 h-4 w-4" /> 再開
          </Button>
        )}
        {isThis && (
          <Button
          ...
          >
            <Square className="mr-2 h-4 w-4" /> 終了して記録
          </Button>
        )}
        {isThis && (
          <Button size="lg" variant="destructive" onClick={clear}>
            <RotateCcw className="mr-2 h-4 w-4" /> 破棄
          </Button>
        )}
      </div>
      ...
      <div className="flex items-center justify-center gap-3 pt-2 border-t">
        ...
        <Label htmlFor="rec-sw" className="text-sm">
          終了時に学習記録へ保存する
        </Label>
      </div>
      {record && (
    ...
    <Card className="p-5 mt-3 space-y-4 text-center">
      <Ring progress={progress} running={!!running}>
        <div className="text-4xl font-mono font-bold tabular-nums">
          {fmtMs(isThis ? remainingMs : minutes * 60000)}
        </div>
        <div className="text-xs text-muted-foreground mt-1">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-center gap-2">
            <Button
              size="icon"
              variant="outline"
              onClick={() => setMinutes((v) => Math.max(1, v - 5))}
            >
              -5
            </Button>
            <Input
            ...
            <Button size="icon" variant="outline" onClick={() => setMinutes((v) => v + 5)}>
      ...
      <div className="flex justify-center gap-3 flex-wrap">
        {!isThis ? (
          <Button size="lg" className="px-8" onClick={() => startWith(minutes)}>
            <Play className="mr-2 h-4 w-4" /> 開始
          </Button>
        ) : (
          <>
      ...
      <div className="flex items-center justify-center gap-3 pt-2 border-t">
        ...
        <Label htmlFor="rec-ct" className="text-sm">
          終了時に学習記録へ保存する
        </Label>
      </div>
      {record && (
  ...
  return (
    <Card className="p-5 mt-3 space-y-4 text-center">
      <div className="flex items-center justify-center gap-2 text-sm">
        ...
        <div className="flex gap-1 ml-1">
          {Array.from({ length: 4 }).map((_, i) => (
            <span
              key={i}
              className={cn("h-2 w-2 rounded-full", i < cycles % 4 ? "bg-primary" : "bg-muted")}
            />
          ))}
        </div>
      </div>
      ...
      <Ring progress={progress} running={!!running} tone={mode === "focus" ? "primary" : "warning"}>
        <div className="text-4xl font-mono font-bold tabular-nums">
          {fmtMs(isThis ? remainingMs : focusMin * 60000)}
        </div>
        <div className="text-xs text-muted-foreground mt-1">
          {focusMin}分集中 / {breakMin}分休憩
        </div>
      </Ring>
      ...
      {!isThis && (
        <div className="grid grid-cols-2 gap-3 max-w-sm mx-auto text-left">
          <div>
            <Label className="text-xs">集中(分)</Label>
            <Input
              type="number"
              value={focusMin}
              onChange={(e) => setFocusMin(Math.max(1, +e.target.value))}
            />
          </div>
          <div>
            ...
            <Input
              type="number"
              value={breakMin}
              onChange={(e) => setBreakMin(Math.max(1, +e.target.value))}
            />
          </div>
        </div>
      )}
...
function Breathing() {
  ...
  return (
    <Card className="p-5 mt-3 space-y-4 text-center">
      ...
      <Button size="lg" className="px-8" onClick={() => setRunning((v) => !v)}>
        {running ? "停止" : "開始"}
      </Button>
    </Card>
  );
