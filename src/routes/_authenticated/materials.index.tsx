...
function MaterialsPage() {
  ...
  const load = async () => {
    ...
    if (q)
      qry = qry.or(
        `title.ilike.%${q}%,author.ilike.%${q}%,publisher.ilike.%${q}%,isbn.eq.${q},barcode.eq.${q}`,
      );
  ...
  const toggleFav = async (m: any, e: React.MouseEvent) => {
    ...
    if (m._fav) {
      ...
      await (supabase as any)
        .from("material_favorites")
        .insert({ user_id: user.id, material_id: m.id });
    }
    load();
  };
  useEffect(() => {
  ...
  const sorted = [...items].sort((a: any, b: any) => {
    ...
    if (sortBy === "usage")
      return (usage[b.id]?.total_minutes ?? 0) - (usage[a.id]?.total_minutes ?? 0);
    return 0;
  });

  const myTotals = Object.values(usage).reduce(
    (acc: any, u: any) => {
    ...
    <div className="container mx-auto p-6 max-w-6xl">
      ...
      <Card className="p-3 mb-4 flex flex-col sm:flex-row gap-2">
        <div className="flex-1 flex gap-2">
          ...
          <Input
            placeholder="タイトル / 著者 / 出版社 / ISBN / バーコード"
            value={q}
      ...
      {myTotals.materials > 0 && (
        <Card className="p-3 mb-4 flex flex-wrap items-center gap-4 text-sm">
          ...
          <div>
            使用中: <b className="tabular-nums">{myTotals.materials}</b> 冊
          </div>
          <div>
            累計: <b className="tabular-nums">{myTotals.minutes}</b> 分
          </div>
        </Card>
      )}
        ...
        {sorted.map((m) => (
          <Link key={m.id} to="/materials/$id" params={{ id: m.id }} className="block">
            <Card className="p-3 hover:bg-accent/40 transition flex gap-3">
              {m.cover_url ? (
                <img src={m.cover_url} alt={m.title} className="w-16 h-20 object-cover rounded" />
              ) : (
                <div className="w-16 h-20 bg-muted rounded flex items-center justify-center text-xs">
                  No img
                </div>
              )}
              <div className="flex-1 min-w-0">
                <div className="flex items-start gap-1">
                  ...
                  <button onClick={(e) => toggleFav(m, e)} className="shrink-0" title="お気に入り">
                    <Star
                      className={`h-4 w-4 ${m._fav ? "fill-amber-400 text-amber-400" : "text-muted-foreground"}`}
                    />
                  </button>
                </div>
                {m.subtitle && (
                ...
                <div className="flex gap-1 flex-wrap mt-1">
                  ...
                  {usage[m.id] && (
                    <Badge variant="outline" className="text-[10px] gap-1">
                      ...
                      {usage[m.id].total_minutes}分 / {usage[m.id].daily_avg}分平均
                    </Badge>
                  )}
                </div>
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>バーコード読み取り</DialogTitle>
        </DialogHeader>
        <div className="space-y-2">
          <div className="relative bg-black rounded aspect-[4/3] overflow-hidden">
            <video
              ...
              playsInline
              muted
              className={`w-full h-full object-cover ${scanning ? "" : "opacity-30"}`}
            />
            {!scanning && (
            ...
            {scanning && (
              <div className="absolute inset-0 border-4 border-primary/60 pointer-events-none" />
            )}
            {scanning && (
              <button
...
function AddDialog({ onSaved }: { onSaved: () => void }) {
  ...
  const save = async () => {
    ...
    ["year", "pages", "difficulty", "price"].forEach((k) => {
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        ...
        {[
          ...
          <div key={k as string} className="col-span-1">
            ...
            <select
              value={f[k as string] ?? ""}
              onChange={(e) => upd(k as string, e.target.value)}
        ...
        <div className="col-span-2">
          ...
          <Textarea
            ...
            onChange={(e) => upd("table_of_contents", e.target.value)}
            rows={4}
          />
        </div>
      </div>
      <Button onClick={save} className="w-full mt-3">
        登録
      </Button>
    </DialogContent>
  );
