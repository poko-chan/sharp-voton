    ...
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      ...
      <Card className="p-6 space-y-3">
        <div className="font-semibold flex items-center gap-2">
          <Link2 className="h-4 w-4" /> 子供アカウントをリンク
        </div>
        <p className="text-xs text-muted-foreground">
          お子様のユーザー名とパスワードを入力すると、認証成功時に紐づけられます。
        </p>
      ...
      {children.length === 0 ? (
        <Card className="p-6 text-sm text-muted-foreground">
          リンクされた子供アカウントはまだありません。
        </Card>
      ) : (
        children.map((c) => <ChildPanel key={c.id} child={c} onChange={reload} unlink={unlink} />)
      )}
    </div>
  );
}
  ...
  return (
    <Card className="p-6 space-y-4">
      <div className="flex items-center gap-3">
        ...
        <Button variant="ghost" size="sm" onClick={unlinkChild}>
            ...
            <table className="w-full text-xs">
              <thead className="bg-muted/40">
                <tr>
                  ...
                  <th className="text-left p-2">分</th>
              ...
              <tbody>
                {(data?.logs ?? []).map((r: any, i: number) => (
                  <tr key={i} className="border-t">
                    ...
                    <td className="p-2">{r.duration_minutes}</td>
    ...
    <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-2 gap-3">
        ...
        <List
          ...
          render={(r: any) => (
            <div className="flex justify-between">
              ...
              <span className="text-muted-foreground truncate ml-2">{r.reason}</span>
            </div>
          )}
        />
        <List
        ...
        <List
          ...
          render={(r: any) => (
            <div className="flex justify-between">
              <span className="truncate">{r.title || "無題"}</span>
              <span className="text-muted-foreground">
        ...
        <List
          ...
          render={(r: any) => (
            <div className="flex gap-2 items-center">
              {r.photo_url && <img src={r.photo_url} className="h-10 w-10 object-cover rounded" />}
              <span className="text-muted-foreground">
        ...
        <List
          title="ミッション"
          rows={d.missions.slice(0, 30)}
          render={(r: any) => (
            <div className="flex justify-between">
              <span>
                {r.date} {r.title ?? r.code}
              </span>
