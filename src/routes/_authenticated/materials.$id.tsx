...
function MaterialDetail() {
  ...
  const load = async () => {
    ...
    if (user) {
      ...
      const { data: gu } = await (supabase as any).rpc("material_global_usage", {
        _material_id: id,
      });
  ...
  const submitEdit = async () => {
    ...
    for (const [k] of FIELDS)
      if (edit[k] !== undefined && edit[k] !== (m[k] ?? "")) patch[k] = edit[k];
    ...
    if (isAdmin) {
      ...
      const { error } = await (supabase as any)
        .from("material_edits")
        .insert({ material_id: id, proposer: user!.id, patch });
      if (error) return toast.error(error.message);
  ...
  const reportThis = async () => {
    ...
    const { error } = await (supabase as any)
      ...
      .insert({ material_id: id, reporter: user!.id, reason: report });
    if (error) return toast.error(error.message);
  ...
  return (
    <div className="container mx-auto p-6 max-w-4xl">
      <Link
        to="/materials"
        className="text-sm text-muted-foreground inline-flex items-center gap-1 mb-3"
      >
      ...
      <Card className="p-4 flex gap-4 mb-4">
        {m.cover_url ? (
          <img src={m.cover_url} alt="" className="w-28 h-36 object-cover rounded" />
        ) : (
          <div className="w-28 h-36 bg-muted rounded" />
        )}
            ...
            m[k] ? (
              <div key={k} className="flex gap-2">
                ...
                <dd className="flex-1 break-words">
                  {Array.isArray(m[k]) ? m[k].join(", ") : String(m[k])}
                </dd>
              </div>
            ) : null,
          )}
      ...
      <Card className="p-4 mb-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-sm">
          <Stat
            ...
            label="あなたの総学習時間"
            value={myUse ? `${myUse.total_minutes}分` : "0分"}
          />
          <Stat
            icon={<CalendarDays className="h-3.5 w-3.5" />}
            label="使用日数 / 1日平均"
            value={myUse ? `${myUse.days_used}日 / ${myUse.daily_avg}分` : "-"}
          />
          <Stat
            icon={<Users className="h-3.5 w-3.5" />}
            label="全体利用者数"
            value={globalUse ? `${globalUse.users_count}人` : "0人"}
          />
          <Stat
            icon={<Clock className="h-3.5 w-3.5" />}
            label="全体総学習時間"
            value={globalUse ? `${globalUse.total_minutes}分` : "0分"}
          />
        </div>
      ...
      <Card className="p-4">
        ...
        <div className="flex gap-2">
          <Input
            placeholder="内容が事実と異なる、不適切など…"
            value={report}
            onChange={(e) => setReport(e.target.value)}
          />
          <Button variant="outline" onClick={reportThis} disabled={!report.trim()}>
            送信
          </Button>
        </div>
      </Card>
