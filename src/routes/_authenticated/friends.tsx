...
function FriendsPage() {
  ...
  const load = async () => {
    ...
    const ids1Pending = (f1 ?? [])
      .filter((r: any) => r.status === "pending")
      .map((r: any) => r.following_id);
    const ids1Blocked = (f1 ?? [])
      .filter((r: any) => r.status === "blocked")
      .map((r: any) => r.following_id);
    const ids2 = (f2 ?? [])
      .filter((r: any) => r.status === "accepted")
      .map((r: any) => r.follower_id);
    const ids2Pending = (f2 ?? [])
      .filter((r: any) => r.status === "pending")
      .map((r: any) => r.follower_id);
    const all = Array.from(
      new Set([...ids1, ...ids2, ...ids1Pending, ...ids2Pending, ...ids1Blocked, user.id]),
    );
    ...
    for (const r of logs ?? [])
      totals.set(r.user_id, (totals.get(r.user_id) ?? 0) + (r.duration_minutes ?? 0));
    const statsMap: Record<string, Stats> = {};
    for (const id of boardIds) statsMap[id] = { minutes7d: totals.get(id) ?? 0 };
    setStats(statsMap);
    const rows = [user.id, ...ids1].map((id) => ({
      ...(map.get(id) as Profile),
  ...
  const doSearch = async () => {
    ...
    setSearching(true);
    const { data } = await supabase.rpc("search_public_profiles", { _q: search });
  ...
  const follow = async (id: string) => {
    ...
    if (error) toast.error(error.message);
      ...
      toast.success("リクエストを送りました（相手の承認待ち）");
      load();
    }
  };
  ...
  const accept = async (id: string) => {
    ...
    await supabase
      .from("follows")
      .upsert(
        { follower_id: user.id, following_id: id, status: "accepted" },
        { onConflict: "follower_id,following_id" },
      );
  ...
  const openDm = (p: Profile) => {
    navigate({ to: "/chat" });
    toast.success(
      `${p.display_name ?? p.username ?? "相手"}とのDMを開くには、チャット一覧から選択してください`,
    );
  };
  ...
  const mutualFriends = useMemo(
    () => following.filter((p) => followerIds.has(p.id)),
  ...
  const relationBadge = (p: Profile) => {
    if (mutualFriends.some((m) => m.id === p.id))
      return (
  ...
  const StatLine = ({ id }: { id: string }) => {
    ...
    return (
      <div className="text-[11px] text-muted-foreground">
        今週の学習: <b className="text-foreground">{s.minutes7d}分</b>
      </div>
    );
  };
  ...
  const FriendActions = ({ p, mutual }: { p: Profile; mutual: boolean }) => (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {followingIds.has(p.id) && (
        <Button size="sm" variant="outline" onClick={() => unfollow(p.id)}>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="flex-wrap h-auto">
          <TabsTrigger value="friends">
            ...
            フレンド ({mutualFriends.length})
          </TabsTrigger>
          <TabsTrigger value="incoming">
            <Inbox className="h-4 w-4 mr-1" />
            申請が来ている ({incomingPending.length})
          </TabsTrigger>
          <TabsTrigger value="outgoing">
            <Send className="h-4 w-4 mr-1" />
            申請中 ({outgoingPending.length})
          </TabsTrigger>
        ...
        <TabsContent value="outgoing" className="space-y-2 mt-4">
          {outgoingPending.map((p) => (
            <Card key={p.id} className="p-3 flex flex-wrap items-center gap-3">
              ...
              <Button size="sm" variant="outline" onClick={() => cancelOutgoing(p.id)}>
                取消
              </Button>
            </Card>
          ))}
            ...
            {searchResults?.map((p) => (
              <Card key={p.id} className="p-3 flex flex-wrap items-center gap-3">
                ...
                {followingIds.has(p.id) ? (
                  ...
                  <Button size="sm" onClick={() => follow(p.id)}>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <Avatar className="h-14 w-14">
                  <AvatarImage src={profileTarget.avatar_url ?? undefined} />
                  <AvatarFallback>{initials(profileTarget)}</AvatarFallback>
                </Avatar>
                <div>
