...
function Mistakes() {
  ...
  const loadEdu = async () => {
    ...
    for (const org of orgIds) {
      const { data: rev } = await (supabase as any).rpc("org_edu_review_rows", {
        _org: org,
        _include_done: false,
      });
      for (const r of (rev ?? []) as any[])
        keyMap.set(r.id, { answer: r.answer, explanation: r.explanation });
    }
    setEdu(
      rows.map((r) => ({
        ...r,
    ...
    (async () => {
      ...
      if (ids.length) {
        const { data } = await supabase
          .from("makron_answers")
          .select("*, makron_questions(prompt)")
          .in("session_id", ids)
          .eq("is_correct", false)
          .order("created_at", { ascending: false })
          .limit(100);
        // 正解はテーブルから直接読めないので、まとめ取得 RPC で一度に取得する
        const { data: keys } = await (supabase as any).rpc("makron_my_answer_keys");
        const answerMap = new Map<string, string>();
        for (const k of (keys ?? []) as any[]) answerMap.set(k.question_id, k.correct_answer ?? "");
        setRows(
          (data ?? []).map((row: any) => ({
            ...row,
            makron_questions: {
    ...
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold">間違い直しノート</h1>
        </div>
      ...
      <Tabs defaultValue="makron">
        ...
        <TabsContent value="makron" className="space-y-3 pt-3">
          {rows.length === 0 && (
            <Card className="p-6 text-sm text-muted-foreground">まだ誤答はありません。</Card>
          )}
          {rows.map((r, i) => (
            <Card key={r.id} className="p-4 space-y-1">
              <div className="text-xs text-muted-foreground">
                #{i + 1} {new Date(r.created_at).toLocaleDateString("ja-JP")}
              </div>
              <div className="text-sm">
                <b>Q:</b> {r.makron_questions?.prompt}
              </div>
              <div className="text-sm text-red-500">
                <b>あなた:</b> {r.answer}
              </div>
              <div className="text-sm text-green-600">
                <b>正解:</b> {r.makron_questions?.answer}
              </div>
              <AiBlock
                row={r}
                text={stream[r.id]}
                ...
                onRun={() =>
                  explain(
                    r,
                    r.makron_questions?.prompt ?? "",
                    r.makron_questions?.answer ?? "",
                    r.answer ?? "",
                  )
                }
              />
        ...
        <TabsContent value="edu" className="space-y-3 pt-3">
          <p className="text-xs text-muted-foreground">
            学校・塾の教材（Makron for
            education）で間違えた問題です。ポイントは付きませんが、AIが解き方を解説します。
          </p>
          {edu.length === 0 && (
            <Card className="p-6 text-sm text-muted-foreground">復習待ちの問題はありません。</Card>
          )}
          {edu.map((r, i) => (
            <Card key={r.id} className="p-4 space-y-1">
              <div className="text-xs text-muted-foreground">
                #{i + 1} {r.org_edu_units?.title} ・{" "}
                {new Date(r.created_at).toLocaleDateString("ja-JP")}
              </div>
              <div className="text-sm">
                <b>Q:</b> {r.org_edu_questions?.body}
              </div>
              <div className="text-sm text-red-500">
                <b>あなた:</b> {r.user_answer}
              </div>
              <div className="text-sm text-green-600">
                <b>正解:</b> {r.org_edu_questions?.answer}
              </div>
              ...
              <AiBlock
                row={r}
                text={stream[r.id] ?? r.ai_review}
                busy={busy === r.id}
                onRun={() =>
                  explain(
                    r,
                    r.org_edu_questions?.body ?? "",
                    r.org_edu_questions?.answer ?? "",
                    r.user_answer ?? "",
                  )
                }
              />
