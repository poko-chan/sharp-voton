...
export const Route = createFileRoute("/_authenticated/makron/result/$sessionId")({
  component: ResultPage,
});

const QCOLS =
  "id, prompt, image_url, type, options, explanation, points, grading, hint_text, order_idx, unit_id, pack_id";
...
function ResultPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const ids: string[] =
        Array.isArray(s.question_ids) && s.question_ids.length
          ? s.question_ids
      ...
      const { data: rev } = await (supabase as any).rpc("makron_reveal", {
        _session_id: sessionId,
      });
      const km: Record<string, string[]> = {};
      (rev ?? []).forEach((r: any) => {
        const list = (
          r.accepted_answers?.length
            ? r.accepted_answers
            : r.correct_options?.length
              ? r.correct_options
              : [r.model_answer].filter(Boolean)
        ) as string[];
  ...
  const byQ: Record<string, any> = useMemo(
    () => Object.fromEntries(answers.map((a) => [a.question_id, a])),
    [answers],
  );

  const display = (val: any) =>
    Array.isArray(val) ? val.join(", ") : val == null || val === "" ? "（未回答）" : String(val);
  ...
  const prompt = useMemo(() => {
    ...
    lines.push(
      `各問題について、なぜ正解／不正解だったか、考え方のポイント、次に学習すべきことを${tone}でまとめてください。`,
    );
    return lines.join("\n");
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-5">
        <Card className="p-6 text-center bg-gradient-to-br from-primary/10 to-background">
          ...
          <div className="text-5xl font-extrabold tabular-nums">
            ...
            <span className="text-2xl text-muted-foreground"> / {session.total_points ?? 0}</span>
          </div>
        ...
        <div className="space-y-3">
          {questions.map((q, i) => {
            ...
            const correct = isManual
              ? a?.manual_score != null && a.manual_score >= q.points
              : a?.auto_correct;
              ...
              <Card key={q.id} className="p-4 space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <div className="font-bold flex items-center gap-2">
                    {noManual ? (
                      <span className="text-xs px-2 py-0.5 rounded bg-muted">手動採点待ち</span>
                    ) : correct ? (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {noManual
                        ? "—"
                        : (a?.awarded_points ??
                          (isManual ? a?.manual_score : correct ? q.points : 0) ??
                          0)}{" "}
                      / {q.points} 点
                    </div>
                  </div>
                ...
                {q.image_url && (
                  <img src={q.image_url} className="max-h-48 rounded border" alt="" />
                )}
                <div className="grid sm:grid-cols-2 gap-2 text-xs">
                  <div className="rounded border p-2">
                    <div className="text-muted-foreground mb-0.5">あなたの解答</div>
                    <div className="whitespace-pre-wrap">{display(a?.answer)}</div>
                  </div>
                  <div className="rounded border p-2">
                    <div className="text-muted-foreground mb-0.5">模範解答</div>
                ...
                {isManual && a?.manual_comment && (
                  <div className="text-xs bg-blue-500/10 rounded p-2 whitespace-pre-wrap">
                    <span className="font-medium">講評: </span>
                    {a.manual_comment}
                  </div>
                )}
