...
function AdminPage() {
  ...
  const loadReports = async () => {
    const { data } = await (supabase as any)
      .from("makron_reports")
      .select(
        "*, question:makron_questions(id, prompt, type, options, explanation, unit_id, pack_id, image_url)",
      )
  ...
  useEffect(() => {
    if (selUnit) loadQuestions(selUnit);
  }, [selUnit]);

  if (!user) {
    return (
      <MakronShell back="/makron/units" title="管理者画面">
        <div className="p-6 text-sm">ログインしてください</div>
      </MakronShell>
    );
  }
  if (!isAdmin) {
    return (
      <MakronShell back="/makron/units" title="管理者画面">
        <div className="p-6 text-sm text-muted-foreground">この画面は管理者のみ利用できます。</div>
      </MakronShell>
    );
  }
  ...
  const addField = async () => {
    ...
    const { error } = await (supabase as any)
      .from("makron_fields")
      .insert({ name: newField.trim(), subject_id: selSubj });
    if (error) return toast.error(error.message);
    setNewField("");
    loadLabels();
  };
        ...
        <Tabs defaultValue="packs">
          <TabsList>
            ...
            <TabsTrigger value="reports">
              ...
              報告 ({reports.filter((r) => r.status === "open").length})
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="packs" className="space-y-4">
            <Card className="p-4 space-y-2">
              ...
              <div className="text-[11px] text-muted-foreground">
                学年・教科・分野・単元はすべて任意です。何も選ばずに単体のパックとして作成できます。
              </div>
              <Input
                placeholder="パック名（例：中1 一次方程式 基礎）"
                value={pTitle}
              ...
              <div className="grid sm:grid-cols-4 gap-2">
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder="分野（任意）" />
                  </SelectTrigger>
                  <SelectContent>
                    {fields
                      .filter((f) => f.subject_id === pSubj)
                      .map((f) => (
                ...
                <Select value={pUnit} onValueChange={setPUnit}>
                  <SelectTrigger>
                    <SelectValue placeholder="単元（任意）" />
                  </SelectTrigger>
                  <SelectContent>
                    {units
                      .filter(
                        (u) =>
                          (!pSubj || u.subject_id === pSubj) && (!pField || u.field_id === pField),
                      )
                      .map((u) => (
              ...
              />
              <Button onClick={createPack} disabled={!pTitle.trim()}>
                作成
              </Button>
            </Card>
            ...
            <Card className="p-4 space-y-1">
              ...
              {packs.length === 0 && (
                <div className="text-xs text-muted-foreground">まだパックがありません</div>
              )}
              {packs.map((p) => (
                    ...
                    <div className="text-[10px] text-muted-foreground">
                      {[
                        ...
                        subjects.find(
                          (s) =>
                            s.id ===
                            (p.subject_id ?? units.find((u) => u.id === p.unit_id)?.subject_id),
                        )?.name,
                        fields.find(
                          (f) =>
                            f.id ===
                            (p.field_id ?? units.find((u) => u.id === p.unit_id)?.field_id),
                        )?.name,
          ...
          <TabsContent value="labels" className="space-y-4">
            <div className="text-xs text-muted-foreground flex items-center gap-1">
              ...
              教科 → 分野 → 単元
              の順に作成します。単元を開くとパック（問題のまとまり）を追加できます。
            </div>
              ...
              <Card className="p-4 space-y-2">
                ...
                <div className="space-y-1 max-h-80 overflow-auto">
                  {subjects.map((s) => (
                    <div
                      key={s.id}
                      className={`flex items-center gap-1 p-2 rounded cursor-pointer ${selSubj === s.id ? "bg-primary/10" : "hover:bg-accent"}`}
                  ...
                  {subjects.length === 0 && (
                    <div className="text-xs text-muted-foreground">教科がまだありません</div>
                  )}
                </div>
              </Card>
                ...
                <div className="font-bold flex items-center gap-1">
                  ...
                  {selSubj && (
                    <span className="text-[10px] text-muted-foreground">
                      （{subjects.find((s) => s.id === selSubj)?.name}）
                    </span>
                  )}
                </div>
                {!selSubj && (
                  <div className="text-xs text-muted-foreground">左から教科を選択してください</div>
                )}
                {selSubj && (
                  <>
                    <div className="flex gap-1">
                    ...
                    <div className="space-y-1 max-h-80 overflow-auto">
                      {subjFields.map((f) => (
                        <div
                          key={f.id}
                          className="flex items-center gap-1 p-2 rounded hover:bg-accent"
                        >
                          <span className="flex-1 truncate">{f.name}</span>
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => renameField(f.id, f.name)}
                          >
                            <Pencil className="h-4 w-4" />
                          </Button>
                          <Button
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <Card className="p-4 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-1">
                  ...
                  <Select value={uFieldId} onValueChange={setUFieldId} disabled={!uSubjectId}>
                    ...
                    <SelectContent>
                      {fields
                        .filter((f: any) => f.subject_id === uSubjectId)
                        .map((f: any) => (
                          <SelectItem key={f.id} value={f.id}>
                            {f.name}
                          </SelectItem>
                ...
                />
                <Button onClick={createUnit} className="w-full" disabled={!uTitle.trim()}>
                  作成
                </Button>
              </Card>
              ...
              <Card className="p-4 space-y-2 max-h-96 overflow-auto">
                ...
                {units.map((u) => (
                  <div key={u.id} className="flex items-center gap-1 p-2 rounded hover:bg-accent">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] text-muted-foreground">
                        {[u.subject, u.field].filter(Boolean).join(" / ") || "未分類"}
                      </div>
                    </div>
                    <Link to="/makron/unit/$unitId" params={{ unitId: u.id }}>
                      <Button size="sm" variant="outline" title="パックを管理">
                        パック
                        <ChevronRight className="h-3 w-3" />
                      </Button>
                    </Link>
                ...
                {units.length === 0 && (
                  <div className="text-xs text-muted-foreground">まだ単元がありません</div>
                )}
              </Card>
            </div>
          </TabsContent>
            ...
            <Card className="p-3 flex flex-wrap items-center gap-2">
              ...
              <Select value={selUnit ?? ""} onValueChange={(v) => setSelUnit(v)}>
                <SelectTrigger className="w-72">
                  <SelectValue placeholder="パックを選択" />
                </SelectTrigger>
                <SelectContent>
                  {packs.map((p) => (
              ...
              <Card className="p-4 space-y-3">
                ...
                <div className="space-y-1 max-h-[50vh] overflow-auto">
                  ...
                  {questions.map((q) => (
                    <div
                      key={q.id}
                      className={`flex items-center gap-1 border rounded p-2 text-sm ${q.is_active === false ? "opacity-50 line-through" : ""}`}
                    >
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        title={q.is_active === false ? "有効化" : "停止"}
                        onClick={async () => {
                  ...
                  <Card className="p-4 space-y-3 bg-muted/30">
                    ...
                    <div className="grid md:grid-cols-3 gap-2">
                        ...
                        <Select
                          ...
                          onValueChange={(v) =>
                            setDraft({
                              ...draft,
                              type: v,
                              options: ["single", "multi"].includes(v)
                      ...
                      <div>
                        ...
                        <Select
                          value={draft.grading}
                          onValueChange={(v) => setDraft({ ...draft, grading: v })}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem
                              value="auto"
                              disabled={!["single", "multi", "text"].includes(draft.type)}
                            >
                              自動
                            </SelectItem>
                    ...
                    <div>
                      ...
                      {draft.image_url && (
                        <img
                          src={draft.image_url}
                          alt=""
                          className="max-h-32 mt-1 border rounded"
                        />
                      )}
                    </div>
                              ...
                              <input
                                ...
                                onChange={() => {
                                  ...
                                  if (draft.type === "single")
                                    setDraft({ ...draft, correct_options: [o] });
                                  else
                                    setDraft({
                                      ...draft,
                                      correct_options: checked
                                        ? draft.correct_options.filter((x: string) => x !== o)
                                        : [...draft.correct_options, o],
                                    });
                                }}
                              />
                              <Input
                                ...
                                onChange={(e) => {
                                  ...
                                  const co = draft.correct_options.map((x: string) =>
                                    x === prev ? e.target.value : x,
                                  );
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  setDraft({
                                    ...draft,
                                    options: opts,
                                    correct_options: draft.correct_options.filter(
                                      (x: string) => x !== draft.options[i],
                                    ),
                                  });
                                }}
                              >
                    ...
                    {draft.type === "text" && (
                      <div>
                        <label className="text-xs">
                          正答（複数可、1行1つ。大文字小文字・前後空白は無視）
                        </label>
                        <Textarea
                          rows={3}
                          value={(draft.accepted_answers ?? []).join("\n")}
                          onChange={(e) =>
                            setDraft({ ...draft, accepted_answers: e.target.value.split("\n") })
                          }
                        />
                      </div>
                    )}
                    ...
                    <div>
                      ...
                      <Textarea
                        rows={2}
                        placeholder="生徒がヒント券を消費すると表示されます。AIは使わず、ここに作成者が記入。"
                        value={draft.hint_text ?? ""}
                        onChange={(e) => setDraft({ ...draft, hint_text: e.target.value })}
                      />
                    </div>
              ...
              <Card key={r.id} className={`p-3 ${r.status === "open" ? "" : "opacity-60"}`}>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <div className="ml-auto flex gap-1">
                    <Button size="sm" variant="outline" onClick={() => setReportDetail(r)}>
                      詳細
                    </Button>
                    {r.status === "open" ? (
                      <Button
                ...
                {reportDetail && (
                  <div className="space-y-3 text-sm">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded ${reportDetail.status === "open" ? "bg-destructive/10 text-destructive" : "bg-success/15 text-success"}`}
                      >
                        {reportDetail.status === "open" ? "未対応" : "対応済"}
                      </span>
                      <span className="text-[11px] text-muted-foreground ml-auto">
                        {new Date(reportDetail.created_at).toLocaleString("ja-JP")}
                      </span>
                    </div>
                    ...
                    <Card className="p-3 space-y-1">
                      ...
                      <div className="whitespace-pre-wrap">
                        {reportDetail.question?.prompt ?? "(削除済み)"}
                      </div>
                      {reportDetail.question?.image_url && (
                        <img
                          src={reportDetail.question.image_url}
                          alt=""
                      ...
                      {(reportDetail.question?.options ?? []).length > 0 && (
                        <ul className="text-xs pl-4 list-disc">
                          {reportDetail.question.options.map((o: string, i: number) => (
                            <li key={i}>{o}</li>
                          ))}
                        </ul>
                      )}
                      {reportDetail.question?.explanation && (
                        <div className="text-xs text-muted-foreground">
                          解説: {reportDetail.question.explanation}
                        </div>
                      )}
                      {reportDetail.question?.unit_id && (
                        <Link
                          to="/makron/unit/$unitId"
                          params={{ unitId: reportDetail.question.unit_id }}
                        >
                    ...
                    {reportDetail.suggested_answer && (
                      <div>
                        ...
                        <div className="whitespace-pre-wrap">{reportDetail.suggested_answer}</div>
                      </div>
                    )}
                    {reportDetail.note && (
                      <div>
                        <div className="text-[11px] text-muted-foreground">コメント</div>
                        <div className="whitespace-pre-wrap">{reportDetail.note}</div>
                      </div>
                    )}
                    <div className="text-[11px] text-muted-foreground">
                      報告者ID: {reportDetail.user_id?.slice(0, 8)}…
                    </div>
                          ...
                          onClick={async () => {
                            try {
                              const { data } = await (supabase as any)
                                .from("makron_questions")
                                .select(QUESTION_COLUMNS)
                                .eq("id", reportDetail.question.id)
                                .maybeSingle();
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          await (supabase as any)
                            .from("makron_reports")
                            .update({ status: next })
                            .eq("id", reportDetail.id);
                          setReportDetail({ ...reportDetail, status: next });
                          loadReports();
                        }}
                      >
                        <FlagOff className="h-3 w-3 mr-1" />
                        {reportDetail.status === "open" ? "対応済みにする" : "未対応に戻す"}
                      </Button>
                    </div>
                  </div>
                )}
