            ...
            <div className="flex items-center gap-1">
              ...
              <Button
                ...
                onClick={async () => {
                  if (
                    !confirm(
                      `シリーズ「${s.name}」を削除しますか？（中の試験はシリーズなしになります）`,
                    )
                  )
                    return;
...
function ExamList({ items, onDeleted }: { items: any[]; onDeleted: () => void }) {
  if (items.length === 0)
    return <Card className="p-4 text-center text-xs text-muted-foreground">試験はありません</Card>;
...
function SeriesDialog({ onSaved }: { onSaved: () => void }) {
  ...
  const save = async () => {
    ...
    const { error } = await (supabase as any)
      .from("exam_series")
      .insert({ user_id: user.id, name: name.trim(), note: note || null });
    if (error) return toast.error(error.message);
