...
export const Route = createFileRoute("/_authenticated/feed")({
  head: () => {
    ...
    const description =
      "Study# のタイムライン。勉強記録を投稿して、いいねやコメントで仲間とモチベーションを高め合えます。";
    return {
      meta: [
        { title },
...
function FeedPage() {
  ...
  const loadProfilesAndSocial = async (rows: FeedPost[]) => {
    ...
    if (uids.length) {
      ...
      setProfiles((prev) => ({
        ...prev,
        ...Object.fromEntries((pr ?? []).map((p: any) => [p.id, p])),
      }));
    }
    if (ids.length) {
  ...
  useEffect(() => {
    load(0, false); /* eslint-disable-next-line */
  }, [scope, user?.id]);
  ...
  useEffect(() => {
    ...
    (supabase as any)
      ...
      .eq("user_id", user.id)
      .then(({ data }: any) =>
        setOrgs((data ?? []).map((m: any) => m.organization).filter(Boolean)),
      );
  }, [user?.id]);
  ...
  const toggleLike = async (id: string) => {
    ...
    setLikes({
      ...likes,
      [id]: { count: (cur?.count ?? 0) + (cur?.mine ? -1 : 1), mine: !cur?.mine },
    });
    if (cur?.mine)
    ...
    if (query.trim()) {
      ...
      rows = rows.filter(
        (p) =>
          ...
          name(p.user_id).toLowerCase().includes(q),
      );
    }
    if (sortMode === "popular") {
        ...
        <div className="min-w-0 space-y-4">
          <div className="sticky top-0 z-10 -mx-4 md:-mx-6 px-4 md:px-6 py-3 bg-background/95 backdrop-blur border-b space-y-3">
            <div className="flex items-center justify-between gap-2">
              <h1 className="text-xl md:text-2xl font-bold flex items-center gap-2 shrink-0">
        ...
        <aside className="hidden lg:block space-y-4">
          ...
          <Card className="p-4 space-y-2">
            ...
            {topSubjects.length === 0 && (
              <div className="text-xs text-muted-foreground">記録がまだありません</div>
            )}
            {topSubjects.map((s) => (
