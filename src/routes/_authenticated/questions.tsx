...
function QuestionsPage() {
  ...
  const [result, setResult] = useState<{ correct: number; total: number; title: string } | null>(
    null,
  );
  ...
  const [examFormats, setExamFormats] = useState<Array<"multiple_choice" | "exam" | "flashcard">>([
    "multiple_choice",
    "exam",
  ]);
      ...
      <div className="p-6 max-w-3xl mx-auto space-y-5">
        ...
        {result ? (
          ...
          <PracticeSession
            questions={sessionQs}
            onDone={(s) => {
              setResult({ ...s, title: sessionTitle });
              qc.invalidateQueries({ queryKey: ["questions"] });
            }}
          />
        )}
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">
          トピックを入力するとAIが問題を作成。生成後はそのまま演習を開始します。
        </p>
      </div>
      ...
      <Card className="p-5 space-y-4 border-primary/30">
        ...
        <p className="text-xs text-muted-foreground">
          試験時間や出題範囲を詳細に入力すると、AIが試験形式の予想問題セットを作成します。
        </p>
        ...
        <div className="grid md:grid-cols-2 gap-3">
          <div>
            ...
            <Input
              ...
              placeholder="例: 日本史B / 微分積分 / TOEIC Part5"
            />
          </div>
          <div>
            <Label>試験時間（分）</Label>
        ...
        <div className="grid md:grid-cols-3 gap-3">
          <div>
            ...
            <Select
              ...
              onValueChange={(v) => setExamDifficulty(v as typeof examDifficulty)}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              {(
                ...
                <label key={v} className="flex items-center gap-1.5 text-sm cursor-pointer">
                  <Checkbox
                    checked={examFormats.includes(v)}
                    onCheckedChange={() => toggleFmt(v)}
                  />
                  {l}
                </label>
              ))}
      ...
      <div className="space-y-3">
        ...
        {groups.length === 0 && (
          <p className="text-sm text-muted-foreground">まだ問題がありません。</p>
        )}
        {groups.map((g) => (
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground mt-1">
                  {g.items.length} 問
                  {g.wrongCount > 0 && (
                    <span className="ml-2 text-destructive">要復習 {g.wrongCount}</span>
                  )}
                </div>
              </div>
              <div className="flex gap-2 shrink-0">
                ...
                <ConfirmDialog
                  trigger={
                    <Button variant="outline" size="icon" title="このトピックを全削除">
                  ...
                  onConfirm={async () => {
                    try {
                      await delTopic({ data: { topic: g.topic } });
              ...
              <ul className="mt-2 space-y-1">
                {g.items.map((q) => (
                  <li key={q.id} className="flex items-start gap-2 p-2 rounded border">
                    ...
                    <ConfirmDialog
                      trigger={
                        <button className="text-destructive hover:opacity-70 shrink-0">
