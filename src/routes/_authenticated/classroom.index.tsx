...
function ClassroomList() {
  ...
  const [classes, setClasses] = useState<
    (ClassRow & { role: string; memberCount: number; pending: number })[]
  >([]);
  ...
  const load = async () => {
    ...
    const studentIds = ids.filter(
      (id) =>
        roleMap.get(id) !== "teacher" &&
        (cls ?? []).find((c: any) => c.id === id)?.owner_id !== user.id,
    );
    ...
    if (studentIds.length > 0) {
      ...
      (asg ?? []).forEach((a: any) => {
        if (!submittedSet.has(a.id))
          pendingMap.set(a.class_id, (pendingMap.get(a.class_id) ?? 0) + 1);
      });
    }
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <Dialog open={openJoin} onOpenChange={setOpenJoin}>
            ...
            <DialogContent>
              ...
              <Input
                ...
                maxLength={12}
              />
              <Button onClick={submitJoin} className="w-full mt-2">
                参加
              </Button>
            </DialogContent>
          </Dialog>
          <Dialog open={openCreate} onOpenChange={setOpenCreate}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>クラスを作成</DialogTitle>
              </DialogHeader>
      ...
      {!loading && classes.length === 0 && (
        <Card className="p-12 text-center space-y-3">
          ...
          <p className="text-muted-foreground text-sm">
            クラスを作成するか、先生から受け取った招待コードで参加しましょう。
          </p>
