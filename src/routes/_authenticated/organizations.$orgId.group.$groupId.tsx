...
export const Route = createFileRoute("/_authenticated/organizations/$orgId/group/$groupId")({
  component: GroupPage,
});
...
function GroupPage() {
  ...
  const load = async () => {
    ...
    if (g) {
      ...
      setColor(g.color);
      setPerms({ ...DEFAULT_PERMS, ...(g.perms ?? {}) });
    }
  ...
  const save = async () => {
    const { error } = await (supabase as any)
      .from("org_groups")
      .update({ name, description: desc || null, color, perms })
      .eq("id", groupId);
    if (error) return toast.error(error.message);
              ...
              .map((m) => (
                <div key={m.user_id} className="flex items-center gap-2 border rounded p-2 text-sm">
                  <div className="flex-1">
                    {nameOf(profiles[m.user_id])}{" "}
                    <span className="text-[10px] px-1.5 rounded bg-muted">
                      {ROLE_LABEL[m.role] ?? m.role}
                    </span>
                  </div>
                  <Select
                    value={picked[m.user_id] ?? "none"}
                    onValueChange={(v) =>
                      setPicked((p) => {
                        const n = { ...p };
                        if (v === "none") delete n[m.user_id];
                        else n[m.user_id] = v;
                        return n;
                      })
                    }
      ...
      <Tabs defaultValue="members">
          ...
          {members.map((m) => (
            <Card key={m.id} className="p-3 flex items-center gap-2 text-sm">
              ...
              <Button
                ...
                onClick={async () => {
                  await (supabase as any).from("org_group_members").delete().eq("id", m.id);
                  load();
                }}
              >
              ...
              {orgMembers
                ...
                .map((om) => (
                  ...
                  >
                    ...
                    <Button size="sm" variant="outline" onClick={() => addMember(om.user_id)}>
                      追加
                    </Button>
                  </div>
                ))}
        ...
        <TabsContent value="chat">
          <p className="text-xs text-muted-foreground mb-2">
            教師以上の権限を持つ人は、グループのチャット内容を閲覧できます。
          </p>
