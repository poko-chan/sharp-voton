...
function MakronUnits() {
  ...
  const visibleUnits = units.filter(
    (u) =>
      (fSubj === "__all" || u.subject_id === fSubj) &&
      (fField === "__all" || u.field_id === fField),
  );
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <MakronBadge
            icon={Trophy}
            label="順位"
            value={me && me.rank > 0 ? `${me.rank}位 / ${me.total_users}人` : "未参加"}
          />
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2 space-y-3">
            <h2 className="text-lg font-bold flex items-center gap-2">
              ...
              問題パック ({filtered.length}/{packs.length})
            </h2>
          ...
          <div className="space-y-3">
            ...
            <Card className="divide-y divide-border/60">
              ...
              {board.map((r) => (
                <div
                  key={r.user_id}
                  className={`flex items-center gap-2 p-2 ${r.user_id === user?.id ? "bg-primary/10" : ""}`}
                >
