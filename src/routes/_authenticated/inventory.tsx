...
function InventoryPage() {
  ...
  const sell = async (code: string, qty = 1) => {
    ...
    const { data, error } = await (supabase as any).rpc("sell_inventory_item", {
      _item_code: code,
      _qty: qty,
    });
    if (error) return toast.error(error.message);
  ...
  const gift = async () => {
    ...
    const { error } = await (supabase as any).rpc("gift_inventory_item", {
      _to: giftTo,
      _item_code: giftTarget.code,
      _qty: giftQty,
      _message: null,
    });
    if (error) return toast.error(error.message);
    ...
    <div className="container mx-auto p-6 max-w-4xl">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        {items.map((it) => {
          ...
          const canUse = ["frame", "theme", "title", "chest", "hint", "revive", "boost"].includes(
            it.category,
          );
          return (
            <Card key={it.id} className="p-4 flex flex-col gap-2">
              ...
              <div className="text-[10px] text-muted-foreground">
                カテゴリ: {it.category} ／ 売却 {refund}コイン
              </div>
              <div className="flex gap-1 mt-auto pt-2 flex-wrap">
                {canUse && (
                  <Button size="sm" onClick={() => use(it.item_code)}>
                    使う
                  </Button>
                )}
                ...
                <Dialog
                  open={giftTarget?.code === it.item_code}
                ...
                >
                  <DialogTrigger asChild>
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => setGiftTarget({ code: it.item_code, max: it.quantity })}
                    >
                  ...
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle>アイテムを送る</DialogTitle>
                    </DialogHeader>
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <Input
                          type="number"
                          min={1}
                          max={giftTarget?.max ?? 1}
                          value={giftQty}
                          onChange={(e) =>
                            setGiftQty(
                              Math.max(1, Math.min(giftTarget?.max ?? 1, +e.target.value || 1)),
                            )
                          }
                        />
                      </div>
                      <Button onClick={gift} disabled={!giftTo} className="w-full">
                        送る
                      </Button>
                    </div>
                  </DialogContent>
