...
export const Route = createFileRoute("/_authenticated/organizations/")({
  head: () => {
    ...
    const description =
      "Study# の組織機能。参加コードでメンバーを集め、勉強時間の共有、組織専用のMakron問題集やクラスルーム管理ができます。";
    return {
      meta: [
        { title },
...
function OrgsPage() {
  ...
  const load = async () => {
    ...
    const { data: mems, error } = await (supabase as any)
      ...
      .select(
        "role, suspended, organization:organizations(id, name, description, status, join_code, owner_id)",
      )
      .eq("user_id", user.id);
  ...
  const respond = async (id: string, accept: boolean) => {
    const { error } = await (supabase as any).rpc("org_respond_invitation", {
      _invite_id: id,
      _accept: accept,
    });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-5">
      ...
      <p className="text-sm text-muted-foreground">
        ...
        {isAdmin && (
          <>
            {" "}
            審査は{" "}
            <Link to="/admin" search={{ tab: "orgs" } as any} className="underline">
              管理者ダッシュボード
            </Link>{" "}
            から。
          </>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <Card className="p-4 space-y-2">
          ...
          <p className="text-[11px] text-muted-foreground">
            経営者・共同管理者が承認するとメンバーになります。QRコードからも入力できます。
          </p>
          <QrScannerDialog
            open={qrOpen}
            ...
            <Card key={a.id} className="p-4 space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-muted">
                  {ORG_TYPE_LABEL[a.org_type] ?? a.org_type}
                </span>
                <span
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {a.organization_id && (
                  <Button size="sm" asChild>
                    <Link to="/organizations/$orgId" params={{ orgId: a.organization_id }}>
                      組織を開く
                    </Link>
                  </Button>
                )}
            ...
            <Card key={i.id} className="p-3 flex items-center gap-2">
              <div className="flex-1 min-w-0">
                <div className="font-medium">
                  {i.organization?.name}{" "}
                  <span className="text-[10px] ml-1 px-1.5 rounded bg-muted">
      ...
      <section className="space-y-3">
        ...
        {myOrgs.length === 0 && (
          <Card className="p-6 text-sm text-muted-foreground">まだ所属組織はありません</Card>
        )}
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {myOrgs.map((m: any) => (
            <Card
              key={m.organization?.id}
              className="glass p-4 flex flex-col gap-3 rounded-2xl transition hover:-translate-y-0.5 hover:shadow-lg"
            >
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-1 flex flex-wrap gap-1">
                    ...
                    {m.organization?.status !== "approved" && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-700">
                        承認待ち
                      </span>
                    )}
                  </div>
              ...
              <Button asChild className="mt-auto w-full">
                <Link to="/organizations/$orgId" params={{ orgId: m.organization?.id }}>
                  開く
                </Link>
              </Button>
            </Card>
