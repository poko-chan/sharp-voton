...
function GoalsPage() {
  ...
  const [form, setForm] = useState<{
    ...
    deadline: "",
    scope: "all",
    count_from: toLocalInput(new Date().toISOString()),
  });
  ...
  const load = async () => {
    ...
    list.forEach((g) => {
      if (g.scope === "manual") {
        ...
        const total = (logs ?? [])
          .filter((l) => new Date(l.created_at) >= since)
  ...
  const addManual = async (g: Goal, mins: number) => {
    ...
    const { error } = await supabase
      .from("goals")
      .update({ progress_minutes: newVal } as never)
      .eq("id", g.id);
    if (error) return toast.error(error.message);
    ...
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-3xl font-bold flex items-center gap-2">
          <Trophy className="text-primary" /> 学習目標
        </h1>
      ...
      <Card className="p-5 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2">
            ...
            <Input
              type="datetime-local"
              value={form.count_from}
              onChange={(e) => setForm({ ...form, count_from: e.target.value })}
            />
            <p className="text-xs text-muted-foreground mt-1">
              この日時以降の学習が「全学習記録」モードでカウントされます。
            </p>
          </div>
        </div>
        <div>
          ...
          <Select
            value={form.scope}
            onValueChange={(v) => setForm({ ...form, scope: v as "all" | "manual" })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
      ...
      <div className="space-y-3">
        {goals.length === 0 && (
          <p className="text-muted-foreground text-sm">まだ目標がありません。</p>
        )}
        {goals.map((g) => {
        ...
        <div className="flex-1 min-w-0 w-full">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex gap-1 shrink-0">
              <Button size="sm" variant="ghost" onClick={() => setEditing((v) => !v)} title="編集">
              ...
              <Button size="sm" variant="ghost" onClick={onRemove} title="削除">
          ...
          {editing && (
            <div className="mt-3 grid grid-cols-2 gap-2 rounded-xl border p-3 bg-muted/30">
              <div>
                <Label className="text-xs">目標時間(分)</Label>
              ...
              <div className="col-span-2">
                <Label className="text-xs">計測開始日時</Label>
                <Input
                  type="datetime-local"
                  value={countFrom}
