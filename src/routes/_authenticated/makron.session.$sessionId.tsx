...
export const Route = createFileRoute("/_authenticated/makron/session/$sessionId")({
...
const QCOLS =
  "id, prompt, image_url, type, options, explanation, points, grading, hint_text, order_idx, created_at, is_active, pack_id, unit_id, status";
...
function SessionPage() {
    ...
    (async () => {
      ...
      if (Array.isArray(s.question_ids) && s.question_ids.length > 0) {
        ...
        const order = new Map<string, number>(
          s.question_ids.map((id: string, i: number) => [id, i]),
        );
      ...
      if (p?.question_limit && p.question_limit > 0 && !s.all_mode)
        list = list.slice(0, p.question_limit);
      setQuestions(list);
      ...
      if (user) {
        const [{ data: bm }, { data: lk }, { data: inv }] = await Promise.all([
          ...
          (supabase as any)
            .from("makron_question_likes")
            .select("question_id, liked")
            .eq("user_id", user.id),
          (supabase as any)
            .from("user_inventory")
            .select("quantity")
            .eq("user_id", user.id)
            .eq("item_code", "hint_ticket")
            .maybeSingle(),
        ]);
  ...
  useEffect(() => {
    ...
    if (!target) return;
    (async () => {
      const { data } = await (supabase as any).rpc("makron_match_choices", {
        _question_id: target.id,
      });
  ...
  const toggleLike = async () => {
    ...
    await (supabase as any)
      .from("makron_question_likes")
      .upsert(
        { user_id: user.id, question_id: q.id, liked },
  ...
  const confirmUseHint = async () => {
    if (!q) return;
    const { error } = await (supabase as any).rpc("consume_inventory", {
      _item_code: "hint_ticket",
      _qty: 1,
    });
    if (error) {
  ...
  const confirmFinish = async () => {
    ...
    try {
      ...
      const graded: Record<
        ...
        { score: number; rate: number; feedback: string; good: string[]; improve: string[] }
      > = { ...aiGrades };
      const needsAi = questions.filter(
        (qq) =>
          (qq.type === "written" || qq.type === "long_text") &&
          qq.grading === "auto" &&
          String(answers[qq.id] ?? "").trim() &&
          !graded[qq.id],
      );
      if (needsAi.length > 0) {
        const { data: keys } = await (supabase as any).rpc("makron_model_answers", {
          _session_id: sessionId,
        });
        const modelMap = new Map<string, string>(
          (keys ?? []).map((k: any) => [k.question_id, k.model_answer]),
        );
        for (const qq of needsAi) {
          try {
            graded[qq.id] = await gradeFn({
              data: {
                prompt: qq.prompt,
      ...
      const rpcName =
        session?.kind === "daily" ? "makron_finalize_daily" : "finalize_makron_session";
  ...
  const saveScratchpad = async (dataUrl: string) => {
    const { error } = await (supabase as any).rpc("makron_session_set_scratchpad", {
      _session_id: sessionId,
      _data: dataUrl,
    });
    if (error) return toast.error(error.message);
  ...
  if (!session)
    return (
      <MakronShell back="/makron/units">
        <div className="p-8 text-muted-foreground">読み込み中...</div>
      </MakronShell>
    );
        ...
        <div className="max-w-3xl mx-auto p-6 space-y-3">
          <Card className="p-5 space-y-2">
            ...
            <div className="text-xs text-muted-foreground">
              出題: {questions.length} 問 ・ 満点:{" "}
              {questions.reduce((s, x) => s + (x.points ?? 10), 0)} 点
              {pack.shuffle && " ・ シャッフル"}
              {usingLimit && ` ・ ${pack.question_limit}問抽出`}
              {!pack.is_official && " ・ 報酬なし（非公式）"}
            </div>
            <div className="flex gap-2 pt-2">
              <Button onClick={() => setShowPreview(false)} disabled={questions.length === 0}>
                演習開始
              </Button>
              {usingLimit && pack.allow_all_mode !== false && (
                <Button variant="outline" onClick={switchAllMode}>
                  全問演習モードで開始
                </Button>
              )}
            </div>
            ...
            <ol className="text-sm space-y-1 list-decimal pl-5">
              {questions.map((qq) => (
                <li key={qq.id} className="truncate">
                  <span className="text-[10px] text-muted-foreground mr-1">[{qq.type}]</span>
                  {qq.prompt}
                </li>
              ))}
            </ol>
  ...
  if (!q)
    return (
      <MakronShell back="/makron/units">
        <div className="p-8 text-muted-foreground">問題がありません</div>
      </MakronShell>
    );
  ...
  return (
      ...
      right={
        <div className="flex gap-1">
          ...
          <Button
            ...
            variant="ghost"
            onClick={useHint}
            disabled={!q.hint_text || hintShown.has(q.id)}
            title={`ヒント券で答えのヒントを見る（残り${hintTickets}枚）`}
          >
    ...
    >
        ...
        <Card className="p-5 space-y-4">
          ...
          {hintShown.has(q.id) && q.hint_text && (
            <div className="rounded border border-amber-500/40 bg-amber-500/10 p-3 text-sm">
              <div className="font-bold text-amber-700 dark:text-amber-300 mb-1 flex items-center gap-1">
          ...
          {q.type === "single" && (
            <RadioGroup value={answers[q.id] ?? ""} onValueChange={setAns} className="grid gap-2">
              {q.options.map((o, i) => (
                <Label
                  key={i}
                  className="flex items-center gap-2 border rounded p-3 cursor-pointer hover:bg-accent"
                >
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const ma = (keys ?? []).find((k: any) => k.question_id === q.id)
                        ?.model_answer as string | undefined;
                      const r = await gradeFn({
                        data: {
                          ...
                          max_points: q.points ?? 10,
                          onProgress: (_p: string, chars: number) =>
                            setGradingProgress(`生成中… ${chars}文字`),
                        },
                      });
                ...
                <span className="text-xs text-muted-foreground">
                  {grading
                    ? gradingProgress || "Gemini Nano 起動中…"
                    : "この採点結果が提出時の得点として確定します（未実行なら提出時に自動採点）"}
                </span>
              </div>
              {aiGrades[q.id] && (
                <div className="rounded border bg-muted/30 p-3 text-sm space-y-1">
                  <div className="font-bold">
                    AI採点: {aiGrades[q.id].score} / {q.points}（{aiGrades[q.id].rate}%）
                  </div>
                  ...
                  {aiGrades[q.id].good.length > 0 && (
                    <div className="text-xs">
                      <span className="font-semibold text-emerald-600">良い点:</span>{" "}
                      {aiGrades[q.id].good.join(" / ")}
                    </div>
                  )}
                  {aiGrades[q.id].improve.length > 0 && (
                    <div className="text-xs">
                      <span className="font-semibold text-amber-600">改善点:</span>{" "}
                      {aiGrades[q.id].improve.join(" / ")}
                    </div>
                  )}
                </div>
              )}
          ...
          {q.type === "ordering" &&
            (() => {
              const cur: string[] =
                Array.isArray(answers[q.id]) && answers[q.id].length === q.options.length
                  ? answers[q.id]
                  : [...q.options];
            ...
            {questions.map((_, i) => {
              ...
              return (
                <button
                  key={i}
                  onClick={() => goto(i)}
                  className={`h-7 w-7 text-xs rounded border relative ${i === idx ? "bg-primary text-primary-foreground" : answered ? "bg-success/20" : ""} ${flagged ? "ring-2 ring-amber-500" : ""}`}
                >
                  {i + 1}
                  {flagged && (
      ...
      <AlertDialog
        ...
        onOpenChange={(o) => !submitting && setSubmitConfirmOpen(o)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
