...
const DESC =
  "StudyΩ の組織機能なら、名簿・課題・お知らせ・アンケート・チャット・共有カレンダー・組織専用教材までを1つのポータルで運用できます。役割は4段階、学年やクラスは年度単位で管理。";
...
const APPS = [
  { emoji: "📢", t: "投稿・お知らせ", d: "グループ（チャンネル）ごとに投稿。ピン留め・検索・並び替え・編集/削除に対応し、画像以外のファイルも添付できます。" },
  { emoji: "📝", t: "課題", d: "期限と必須設定つきで配信。提出状況を一覧と統計で把握できます。小テストは正答をサーバー側で採点し、答えは生徒に露出しません。" },
  { emoji: "🗳️", t: "アンケート", d: "Googleフォーム風のUI。セクション分割、必須/任意、条件分岐（回答によって次の質問を変える）まで作成できます。" },
  { emoji: "📅", t: "共有カレンダー", d: "行事・試験・課題期限を月／週／日ビューで共有。" },
  { emoji: "💬", t: "チャット", d: "「やること」「承認待ち」「スレッド」など目的別タブで整理。既読/未読と検索に対応。" },
  { emoji: "🪪", t: "デジタル学生証", d: "組織内で使える身分証を画面に表示。名簿情報と連動します。" },
  { emoji: "👥", t: "名簿・プロフィール", d: "学年・クラス・教室番号など、組織ごとに必要な項目を定義。年度が変わっても履歴を保ったまま更新できます。" },
  { emoji: "🎓", t: "Makron for education", d: "組織限定の問題集。学年などの属性で出題を切り替え、カリキュラム・パスで進捗を表示。誤答はAI復習で解説します。" },
];
...
const FLOW = [
  ...
  { n: "02", t: "承認・初期設定", d: "承認されるとポータルが作られます。プロフィール項目（学年・クラスなど）と年度を設定します。" },
  { n: "03", t: "メンバーを集める", d: "6桁の参加コードを配るか、個別に招待します。既存のアカウントをそのまま所属させられます。" },
    ...
    <div className="relative min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-6xl px-4 py-14 sm:py-20">
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
          <Link to="/login" className="cta">アカウントを作って申請する</Link>
          <Link to="/all-services" className="cta-ghost">全機能を見る</Link>
        </div>
          ...
          <div className="mt-6 grid gap-3 sm:gap-4 md:grid-cols-2 lg:grid-cols-4">
            {APPS.map((a) => (
              <article key={a.t} className="surface surface-hover p-5">
                <span className="grid h-10 w-10 place-items-center rounded-xl bg-primary/10 text-xl" aria-hidden>{a.emoji}</span>
        ...
        <section className="mt-20 grid gap-3 md:grid-cols-3">
          {[
            { t: "生徒の学習量が見える", d: "課題の提出状況だけでなく、学習時間や演習の正答率まで統計で把握できます。" },
            { t: "使いすぎを抑えられる", d: "組織内のメンバーに対して、利用できるサービスをまとめて制限できます。" },
            { t: "答えは見えない設計", d: "小テストや組織教材の正答はサーバー側で採点し、生徒の画面には配信しません。" },
          ].map((c) => (
        ...
        <div className="surface mt-16 p-8 text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">アカウント作成後、組織ページから申請できます。運営が内容を確認して承認します。</p>
          <Link to="/login" className="cta mt-6">はじめる</Link>
        </div>
      </main>
